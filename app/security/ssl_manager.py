...
class SSLManager:
    ...
    def generate_self_signed_certificate(
        ...
        try:
            ...
            logger.info(
                f"🔑 Gerando certificado auto-assinado para: {', '.join(domains)}"
            )
    ...
    def setup_letsencrypt(
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["certbot", "--version"], check=True, capture_output=True
                )
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(f"{letsencrypt_dir}/fullchain.pem"):
                    subprocess.run(
                        ["cp", f"{letsencrypt_dir}/fullchain.pem", str(self.cert_file)]
                    )
                    subprocess.run(
                        ["cp", f"{letsencrypt_dir}/privkey.pem", str(self.key_file)]
                    )
    ...
    def validate_certificate(self, cert_path: str = None) -> Dict:
        ...
        try:
            ...
            result = {
                ...
                "issuer": {
                    "common_name": issuer.get_attributes_for_oid(NameOID.COMMON_NAME)[
                        0
                    ].value,
                    "organization": (
                        issuer.get_attributes_for_oid(NameOID.ORGANIZATION_NAME)[
                            0
                        ].value

...
class CSPViolationReport(BaseModel):
    ...
    document_uri: str = Field(
        ..., description="URI do documento onde ocorreu a violação"
    )
    ...
    column_number: Optional[int] = Field(
        None, description="Número da coluna onde ocorreu"
    )
...
class CSPSecurityService:
    ...
    async def log_csp_violation(
        self, report: CSPViolationReport, client_ip: str
    ) -> None:
    ...
    def _assess_severity(self, report: CSPViolationReport) -> str:
        ...
        if any(
            pattern in report.blocked_uri.lower() for pattern in suspicious_patterns
        ):
            return "high"
    ...
    def _assess_risk_level(self, report: CSPViolationReport) -> str:
        ...
        if (
            ...
            and "javascript:" in report.blocked_uri
        ):
            return "high"
...
async def handle_csp_violation(
    ...
    try:
        ...
        background_tasks.add_task(csp_service.log_csp_violation, csp_report, client_ip)

        return JSONResponse(status_code=204, content={})  # No Content

    except Exception as e:
        logger.error(f"❌ Erro ao processar relatório CSP: {e}")
        raise HTTPException(status_code=500, detail="Error processing CSP report")
...
async def get_csp_violation_stats(hours: int = 24) -> CSPViolationStats:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error retrieving CSP statistics")

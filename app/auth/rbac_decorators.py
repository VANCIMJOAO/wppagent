...
class InsufficientPermissionsException(RBACException):
    ...
    def __init__(self, required_permission: Union[PermissionType, List[PermissionType]]):
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if require_2fa and user.requires_2fa:
                ...
                logger.info(f"2FA required for user {user.id} for permission {permission.value}")
...
def require_any_permission(
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if require_2fa and user.requires_2fa:
                logger.info(f"2FA required for user {user.id} for permissions {[p.value for p in permissions]}")
...
def require_all_permissions(
    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if require_2fa and user.requires_2fa:
                logger.info(f"2FA required for user {user.id} for all permissions {[p.value for p in permissions]}")
...
def require_role(
    role: Union[RoleType, List[RoleType]]
):
    """

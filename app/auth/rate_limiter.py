...
class RateLimiter:
    ...
    def check_rate_limit(
        ...
        results = {"allowed": True, "checks": [], "blocked_by": None, "retry_after": 0}
    ...
    def _check_ddos_protection(self, client_ip: str) -> Dict:
        ...
        if not redis_manager.is_available:
            ...
            return {
                "allowed": True,
                "message": "Redis unavailable, DDoS protection disabled",
            }
        ...
        if request_count is None:
            return {
                "allowed": True,
                "message": "Redis operation failed, allowing request",
            }
    ...
    def _check_ip_rate_limit(self, client_ip: str, endpoint_type: str) -> Dict:
        ...
        if not redis_manager.is_available:
            return {
                "allowed": True,
                "message": "Redis unavailable, IP rate limiting disabled",
            }
        ...
        limit_config = self.limits[RateLimitType.IP].get(
            endpoint_type, self.limits[RateLimitType.IP]["default"]
        )
    ...
    def _check_user_rate_limit(self, user_id: str, endpoint_type: str) -> Dict:
        ...
        if not redis_manager.is_available:
            return {
                "allowed": True,
                "message": "Redis unavailable, user rate limiting disabled",
            }
        ...
        limit_config = self.limits[RateLimitType.USER].get(
            endpoint_type, self.limits[RateLimitType.USER]["default"]
        )
    ...
    def _check_endpoint_rate_limit(self, endpoint: str) -> Dict:
        ...
        if not redis_manager.is_available:
            return {
                "allowed": True,
                "message": "Redis unavailable, endpoint rate limiting disabled",
            }
        ...
        return self._check_sliding_window_limit(
            f"rate:endpoint:{endpoint}", limit_config, f"blocked:endpoint:{endpoint}"
        )

    def _check_global_rate_limit(self, endpoint_type: str) -> Dict:
        """Rate limit global"""
        limit_config = self.limits[RateLimitType.GLOBAL].get(
            endpoint_type, self.limits[RateLimitType.GLOBAL]["total"]
        )

        return self._check_sliding_window_limit(
            f"rate:global:{endpoint_type}", limit_config, "blocked:global"
        )

    def _check_sliding_window_limit(
        self, key: str, limit: RateLimit, block_key: str
    ) -> Dict:
        """Implementa sliding window rate limiting"""
        if not redis_manager.is_available:
            return {
                "allowed": True,
                "message": "Redis unavailable, sliding window rate limiting disabled",
            }
    ...
    def _record_request(
        self, client_ip: str, user_id: Optional[str], endpoint: str, endpoint_type: str
    ):
    ...
    def unblock_ip(self, client_ip: str) -> bool:
        ...
        keys_to_delete = [f"blocked:ip:{client_ip}", f"blocked:ddos:{client_ip}"]
    ...
    def get_rate_limit_status(
        ...
        status = {
            "ip": self._get_limit_status(
                f"rate:ip:{client_ip}", self.limits[RateLimitType.IP]["default"]
            ),
            "blocked_ips": [],
            "blocked_users": [],
        }
        ...
        if user_id:
            status["user"] = self._get_limit_status(
                f"rate:user:{user_id}", self.limits[RateLimitType.USER]["default"]
            )
        ...
        def check_blocks(client):
            ...
            if client.exists(f"blocked:ip:{client_ip}"):
                blocks.append(
                    {"ip": client_ip, "ttl": client.ttl(f"blocked:ip:{client_ip}")}
                )
            return blocks

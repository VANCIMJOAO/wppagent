...
class SecretsManager:
    ...
    def create_secret(
        ...
        # Log de auditoria
        self._log_audit(
            "secret_created", secret_id, {"type": secret_type.value, "version": 1}
        )

        return secret

    def get_secret(
        self, secret_id: str, version: Optional[int] = None
    ) -> Optional[Secret]:
        ...
        if secret_dict["expires_at"]:
            secret_dict["expires_at"] = datetime.fromisoformat(
                secret_dict["expires_at"]
            )

        if secret_dict["rotated_at"]:
            secret_dict["rotated_at"] = datetime.fromisoformat(
                secret_dict["rotated_at"]
            )
    ...
    def rotate_secret(self, secret_id: str, new_value: Optional[str] = None) -> Secret:
        ...
        self._log_audit(
            "secret_rotated",
            secret_id,
            {"old_version": current_secret.version, "new_version": new_version},
        )
    ...
    def revoke_secret(self, secret_id: str, version: Optional[int] = None) -> bool:
        ...
        if version is None:
            ...
            self._log_audit(
                "secret_revoked_all", secret_id, {"versions_revoked": revoked_count}
            )
            ...
            self._log_audit("secret_revoked", secret_id, {"version": version})
        ...
        for key in active_keys:
            ...
            if (
                ...
                try:
                    ...
                    self._log_audit(
                        "secret_rotation_failed", secret_id, {"error": str(e)}
                    )
                ...
                secrets.append(
                    {
                        ...
                        "rotated_at": (
                            secret.rotated_at.isoformat() if secret.rotated_at else None
                        ),
    ...
    def get_audit_log(
        ...
        if secret_id:
            audit_log = [
                entry for entry in audit_log if entry.get("secret_id") == secret_id
            ]

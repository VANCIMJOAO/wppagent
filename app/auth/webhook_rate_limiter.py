...
class WebhookRateLimiter:
            ...
            if self._check_local_cache(key):
                ...
                if cached_result.get("blocked_until", 0) > time.time():
                    return False, {
                        "level": "BLOCKED",
                        "reason": "cached_block",
                        "retry_after": int(cached_result["blocked_until"] - time.time())
                    }
    ...
    async def _check_burst_limit(
        ...
        return await execute_redis_safe(
            _burst_check,
            default=(True, {"status": "redis_unavailable"})
        )
    ...
    async def _escalate_blocking(
        ...
        async def _escalate():
            ...
            escalated_duration = int(base_duration * (1 + violation_count * config.escalation_factor))
        ...
        async def _get_metrics():
            ...
            if results[3]:
                ...
                if blocked_timestamp > current_time:
                    blocked_until = datetime.fromtimestamp(blocked_timestamp, tz=timezone.utc)
            ...
            return WebhookMetrics(
                ...
                current_level=WebhookRateLimitLevel.BLOCKED if blocked_until else WebhookRateLimitLevel.NORMAL,
    ...
    def _update_local_cache(self, key: str, metrics: WebhookMetrics):
        ...
        self._local_cache[key] = {
            "metrics": asdict(metrics),
            "blocked_until": metrics.blocked_until.timestamp() if metrics.blocked_until else 0,
    ...
    async def get_webhook_stats(self) -> Dict[str, Any]:
        ...
        try:
            async def _get_stats():
                ...
                stats = {
                    "total_sources": len(set(k.split(':')[2] for k in keys if k.count(':') >= 2)),
    ...
    async def clear_webhook_blocks(self, source_ip: str, webhook_type: str = "default") -> bool:

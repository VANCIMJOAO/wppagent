...
class BaseConfig(BaseSettings):
    ...
    # APLICAÇÃO
    # ==============================
    app_name: str = Field(default="WhatsApp Agent", alias="APP_NAME")
    ...
    workers: int = Field(default=1, alias="WORKERS", ge=1, le=16)
    ...
    access_token_expire_minutes: int = Field(
        default=30, alias="ACCESS_TOKEN_EXPIRE_MINUTES", ge=1, le=1440
    )
    ...
    admin_password: Optional[SecretStr] = Field(
        default=None, alias="ADMIN_PASSWORD", description="Senha do administrador"
    )
    ...
    whatsapp_api_url: str = Field(
        default="https://graph.facebook.com/v18.0", alias="WHATSAPP_API_URL"
    )

    whatsapp_token: Optional[SecretStr] = Field(
        default=None, alias="WHATSAPP_TOKEN", description="Token de acesso do WhatsApp"
    )

    whatsapp_phone_id: Optional[str] = Field(default=None, alias="WHATSAPP_PHONE_ID")

    whatsapp_verify_token: Optional[SecretStr] = Field(
        default=None, alias="WHATSAPP_VERIFY_TOKEN"
    )
    ...
    meta_api_version: str = Field(
        default="v18.0", alias="META_API_VERSION", description="Versão da Meta API"
    )
    ...
    vapid_public_key_frontend: Optional[str] = Field(
        default=None,
        alias="NEXT_PUBLIC_VAPID_PUBLIC_KEY",
    ...
    database_dsn: Optional[str] = Field(
        default=None, alias="DATABASE_DSN", description="DSN do banco de dados"
    )

    db_host: str = Field(
        default="localhost", alias="DB_HOST", description="Host do banco de dados"
    )

    db_port: int = Field(
        default=5432, alias="DB_PORT", description="Porta do banco de dados"
    )

    db_name: str = Field(
        default="whatsapp_agent", alias="DB_NAME", description="Nome do banco de dados"
    )

    db_user: str = Field(
        default="whatsapp", alias="DB_USER", description="Usuário do banco de dados"
    )
    ...
    database_max_overflow: int = Field(
        default=10, alias="DATABASE_MAX_OVERFLOW", ge=0, le=50
    )
    ...
    redis_enabled: bool = Field(
        default=True, alias="REDIS_ENABLED", description="Habilitar Redis"
    )
    ...
    rate_limit_requests: int = Field(
        default=100, alias="RATE_LIMIT_REQUESTS", ge=1, le=10000
    )
    ...
    # ==============================
    log_dir: str = Field(
        default="logs", alias="LOG_DIR", description="Diretório dos logs"
    )

    log_level: LogLevel = Field(default=LogLevel.INFO, alias="LOG_LEVEL")

    log_format: LogFormat = Field(default=LogFormat.PLAIN, alias="LOG_FORMAT")
    ...
    log_max_size_mb: int = Field(default=50, alias="LOG_MAX_SIZE_MB", ge=1, le=1000)
    ...
    log_backup_count: int = Field(default=5, alias="LOG_BACKUP_COUNT", ge=1, le=50)

    log_request_id: bool = Field(
        default=True, alias="LOG_REQUEST_ID", description="Incluir request ID nos logs"
    )

    log_user_context: bool = Field(
        default=True,
        alias="LOG_USER_CONTEXT",
        description="Incluir contexto do usuário nos logs",
    )

    log_performance: bool = Field(
        default=True,
        alias="LOG_PERFORMANCE",
        description="Habilitar logs de performance",
    )

    log_business: bool = Field(
        default=True, alias="LOG_BUSINESS", description="Habilitar logs de business"
    )
    ...
    prometheus_port: int = Field(
        default=9090, alias="PROMETHEUS_PORT", ge=1024, le=65535
    )
    ...
    sla_error_rate_percentage: float = Field(
        default=1.0,
        alias="SLA_ERROR_RATE_PERCENTAGE",
        ge=0.1,
        le=10.0,
    ...
    alerting_enabled: bool = Field(
        default=False,
        alias="ALERTING_ENABLED",
        description="Habilitar sistema de alertas",
    )

    alert_email: Optional[str] = Field(
        default=None, alias="ALERT_EMAIL", description="Email para receber alertas"
    )
    ...
    alert_response_time_threshold_ms: float = Field(
        default=3000.0,
        alias="ALERT_RESPONSE_TIME_THRESHOLD_MS",
        ge=500.0,
        le=30000.0,
    ...
    alert_error_rate_threshold: float = Field(
        default=5.0,
        alias="ALERT_ERROR_RATE_THRESHOLD",
        ge=0.1,
        le=50.0,
    ...
    max_tokens: int = Field(default=150, alias="MAX_TOKENS", ge=50, le=4000)
    ...
    # TIMEOUTS
    # ==============================
    request_timeout: int = Field(default=30, alias="REQUEST_TIMEOUT", ge=5, le=300)

    llm_timeout: int = Field(default=30, alias="LLM_TIMEOUT", ge=5, le=120)

    webhook_timeout: int = Field(default=10, alias="WEBHOOK_TIMEOUT", ge=1, le=60)
    ...
    backup_interval_hours: int = Field(
        default=24, alias="BACKUP_INTERVAL_HOURS", ge=1, le=168
    )

    backup_retention_days: int = Field(
        default=30, alias="BACKUP_RETENTION_DAYS", ge=1, le=365
    )
    ...
    feature_conversation_memory: bool = Field(
        default=True, alias="FEATURE_CONVERSATION_MEMORY"
    )

    feature_sentiment_analysis: bool = Field(
        default=True, alias="FEATURE_SENTIMENT_ANALYSIS"
    )

    feature_auto_responses: bool = Field(default=True, alias="FEATURE_AUTO_RESPONSES")

    feature_booking_integration: bool = Field(
        default=True, alias="FEATURE_BOOKING_INTEGRATION"
    )
    ...
    def validate_config_consistency(self):
        ...
        if self.environment == Environment.PRODUCTION:
            ...
            if (
                not self.secret_key
                or self.secret_key.get_secret_value() == "your-secret-key"
            ):
        ...
        if self.alerting_enabled and not (self.alert_email or self.alert_slack_webhook):
            ...
            if self.environment in [Environment.DEVELOPMENT, Environment.TEST]:
                ...
                raise ValueError(
                    "At least one alert channel must be configured when alerting is enabled"
                )

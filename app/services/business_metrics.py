...
class BusinessMetricsCollector:
    ...
    async def record_metric(self, metric: BusinessMetric):
        ...
        try:
            ...
            # Log do evento
            log_business_event(
                f"metric_recorded_{metric.metric_type.value}",
                {
    ...
    async def _update_aggregations(self, metric: BusinessMetric):
        ...
        try:
            ...
            for period, key in periods.items():
                agg_key = (
                    f"{metric.metric_type.value}_{metric.name}_{period.value}_{key}"
                )
    ...
    async def record_conversation_started(
        self, user_id: str, channel: str = "whatsapp"
    ):
    ...
    async def record_conversation_completed(
        ...
        self.real_time_counters["active_conversations"] = max(
            0, self.real_time_counters["active_conversations"] - 1
        )
    ...
    async def record_lead_qualified(
        self, user_id: str, qualification_criteria: List[str]
    ):
    ...
    async def record_booking_completed(
        self, user_id: str, service_type: str, revenue: float, booking_id: str
    ):
    ...
    async def record_revenue(
        self, amount: float, source: str, category: str = "general"
    ):
        """Registrar receita"""
        await self.record_metric(
            BusinessMetric(
    ...
    async def record_api_call_cost(
        self, cost: float, api_name: str, tokens_used: int = 0
    ):
    ...
    async def get_real_time_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            conversion_rate = (
                (completed_conv / total_conv * 100) if total_conv > 0 else 0
            )
            ...
            dashboard = {
                ...
                "conversations": {
                    ...
                    "completed": self.real_time_counters.get(
                ...
                "performance": {
                    "avg_response_time": (
                        round(statistics.mean(recent_response_times), 2)
                        if recent_response_times
                        else 0
                    ),
                    "satisfaction_avg": (
                        round(statistics.mean(recent_satisfaction), 2)
                ...
                "business": {
                    ...
                    "avg_lead_score": (
                        round(statistics.mean(recent_lead_scores), 2)
                        if recent_lead_scores
                        else 0
                    ),
                },
                "system": {
    ...
    async def get_period_report(
        self, period: MetricPeriod, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
                ...
                if period.value in agg_key:
                    ...
                    if agg_data["count"] > 0:
                        report["metrics"][metric_name] = {
                            ...
                            "max": agg_data["max"],
                            "median": (
                                statistics.median(agg_data["values"])
                                if agg_data["values"]
                                else 0
                            ),
                        }
    ...
    async def export_metrics(self, start_date: datetime, end_date: datetime) -> str:
        ...
        try:
            ...
            export_file = (
                self.storage_path
                / f"export_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.json"
            )
...
async def track_conversation_end(
    user_id: str, duration_minutes: float, satisfaction: Optional[float] = None
):
    """Função de conveniência para tracking de fim de conversa"""
    await metrics_collector.record_conversation_completed(
        user_id, duration_minutes, satisfaction
    )


async def track_lead_conversion(
    user_id: str, lead_score: float, source: str = "whatsapp"
):
...
async def track_booking_success(
    user_id: str, service_type: str, revenue: float, booking_id: str
):
    """Função de conveniência para tracking de agendamento bem-sucedido"""
    await metrics_collector.record_booking_completed(
        user_id, service_type, revenue, booking_id
    )

...
class SQLOptimizer:
    ...
    async def analyze_file_patterns(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            n_plus_one_issues = self._detect_n_plus_one_patterns(file_path, content, lines)
            issues.extend([{
                "file": file_path,
    ...
    def _detect_n_plus_one_patterns(
        ...
        pattern1 = re.compile(r'for\s+\w+\s+in\s+.*:.*session\.get\(', re.MULTILINE | re.DOTALL)
        ...
        for i, line in enumerate(lines):
            if 'await session.execute(' in line or 'await session.scalar(' in line:
                ...
                if len(sequential_queries) > 3:
                    ...
                    if all(abs(recent_queries[j][0] - recent_queries[j-1][0]) < 5 
        ...
        for i, line in enumerate(lines):
            if 'select(' in line and 'join(' not in line.lower():
                ...
                for j in range(i+1, min(i+10, len(lines))):
                    if any(field in lines[j].lower() for field in ['user_name', 'business_name', 'service_name']):
                        issues.append(QueryIssue(
    ...
    def _detect_inefficient_queries(
        ...
        # Query sem LIMIT
        for i, line in enumerate(lines):
            if 'select(' in line and 'limit(' not in line.lower() and 'count(' not in line.lower():
                issues.append(QueryIssue(
    ...
    async def analyze_route_files(self, file_paths: List[str]) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Analisando {len(file_paths)} arquivos de rotas para padrões N+1")
        ...
        for file_path in file_paths:
            try:
                ...
                self.logger.info(f"✅ Analisado {file_path}: {len(issues)} problemas encontrados")
            except Exception as e:
    ...
    async def generate_optimization_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "medium_impact": [i for i in issues if i.get("estimated_impact") == "medium"],
...
class OptimizedQueryBuilder:
    ...
    def build_appointments_with_relations():
        ...
        return select(
            ...
            Business, Appointment.business_id == Business.id  # INNER JOIN - business sempre existe
        ).outerjoin(
            Service, Appointment.service_id == Service.id  # LEFT JOIN - service pode ser null
        )
    ...
    def build_conversations_with_stats():
        ...
        return select(
            ...
            func.string_agg(
                Message.content.distinct(), 
                ' | ' 
            ).label("recent_messages_preview")
        ).select_from(
            Conversation
        ).join(
    ...
    def build_clients_with_full_stats():
        ...
        return select(
            ...
            func.case([
                (func.max(Message.created_at) > datetime.now() - timedelta(days=30), 'active'),
            ], else_='inactive').label("activity_status")
        ).select_from(
            User
        ).outerjoin(
...
class QueryPerformanceBenchmark:
    ...
    async def benchmark_n_plus_one_fix(
        ...
        avg_n_plus_one = sum(n_plus_one_times) / len(n_plus_one_times) if n_plus_one_times else float('inf')
        avg_optimized = sum(optimized_times) / len(optimized_times) if optimized_times else float('inf')
        
        improvement = ((avg_n_plus_one - avg_optimized) / avg_n_plus_one * 100) if avg_n_plus_one != float('inf') else 0
        ...
        result = {
            ...
            "speedup_factor": avg_n_plus_one / avg_optimized if avg_optimized != 0 else float('inf'),
        ...
        logger.info(f"   Improvement: {improvement:.1f}% ({avg_n_plus_one/avg_optimized:.1f}x faster)")
        ...
        try:
            ...
            for file_path in route_files:
                try:
                    ...
                    all_issues.extend(issues)
                    logger.info(f"✅ Analisado {file_path}: {len(issues)} problemas encontrados")
                except Exception as e:
...
if __name__ == "__main__":
    ...
    print("  python -c 'from app.services.sql_optimizer import analyze_all_routes; import asyncio; asyncio.run(analyze_all_routes())'")

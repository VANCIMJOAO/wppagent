...
class CDNManager:
        ...
        try:
            ...
            if content_type in self.compressible_types:
                ...
                if len(compressed) < len(content):  # Só usar se realmente economizar
                    ...
                    self.metrics["compression_savings"] += len(content) - len(
                        compressed
                    )
    ...
    async def serve_asset(self, request: Request, path: str) -> Response:
        ...
        try:
            ...
            use_compression = (
                "gzip" in accept_encoding and path in self.compressed_cache
            )
            ...
            headers = {
                ...
                "Last-Modified": asset_info.last_modified.strftime(
                    "%a, %d %b %Y %H:%M:%S GMT"
                ),
    ...
    async def preload_critical_assets(self) -> str:
        ...
        """
        critical_assets = ["dashboard.html", "css/dashboard.css", "js/dashboard.js"]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "compression_ratio": (
                round((1 - compressed_size / total_size) * 100, 2)
                if total_size > 0
                else 0
            ),

...
from app.services.retry_handler import CircuitBreakerConfig, retry_handler
...
class WhatsAppService:
    ...
    async def _make_api_request(self, endpoint: str, payload: Dict) -> Dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                endpoint, headers=self.headers, json=payload, timeout=30.0
            )
            ...
            if response.status_code == 401:
                ...
                raise Exception(
                    f"Erro na API ({response.status_code}): {response_data}"
                )
    ...
    async def send_text_message(self, to: str, message: str) -> Dict:
        ...
        try:
            ...
            result = await whatsapp_security.send_message(
                phone_number=to, message=message, message_type="text"
            )
    ...
    def verify_webhook(self, verify_token: str, challenge: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(
                f"Config webhook_verify_token type: {type(config.webhook_verify_token)}"
            )
            ...
            logger.info(
                f"Comparando tokens: recebido='{verify_token}', esperado='{expected_token}'"
            )

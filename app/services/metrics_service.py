...
class MetricsService:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.system_info = Info(
            "whatsapp_bot_system_info", "Informações do sistema", registry=self.registry
        )
    ...
    def track_response_time(
        self, strategy: str, complexity: str = "normal", cache_hit: bool = False
    ):
    ...
    def decrement_active_conversations(self, status: str = "active"):
        ...
        try:
            ...
            if not PROMETHEUS_AVAILABLE:
                self.in_memory_metrics["active_conversations"] = max(
                    0, self.in_memory_metrics["active_conversations"] - 1
                )
    ...
    def track_lead_score(self, score: float, category: str, customer_type: str):
        ...
        try:
            self.lead_scores.labels(
                category=category, customer_type=customer_type
            ).observe(score)
    ...
    def track_strategy_selection(self, strategy: str, reason: str, success: bool):
        ...
        try:
            self.strategy_selection.labels(
                strategy=strategy, reason=reason, success="true" if success else "false"
            ).inc()
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            if self.in_memory_metrics["response_times"]:
                avg_response_time = sum(self.in_memory_metrics["response_times"]) / len(
                    self.in_memory_metrics["response_times"]
                )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics_count": (
                len(self.in_memory_metrics)
                if not PROMETHEUS_AVAILABLE
                else "prometheus_managed"
            ),

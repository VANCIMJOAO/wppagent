...
class OptimizedCacheService:
    ...
    def __init__(self):
        ...
        try:
            ...
            redis_url = os.getenv(
                "REDIS_URL",
                "redis://default:SvSHiMNuuQEtmIUgGIEGqPpXsdZeInDG@yamanote.proxy.rlwy.net:14106",
            )
    ...
    async def get_or_set(
        ...
        try:
            ...
            cache_ttl = ttl or self.ttl_configs.get(
                cache_type, self.ttl_configs["default"]
            )

            # Serializar com handling de tipos especiais
            serialized_data = json.dumps(
                data, default=self._json_serializer, ensure_ascii=False
            )
    ...
    def invalidate_business_cache(self, business_id: int) -> int:
        ...
        logger.info(
            f"🏢 Invalidated {total_deleted} cache entries for business {business_id}"
        )
        return total_deleted
...
def cached(key_template: str, ttl: Optional[int] = None, cache_type: str = "default"):
    """
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            return await cache_service.get_or_set(
                cache_key, fetch_data, ttl=ttl, cache_type=cache_type
            )
...
class CacheKeys:
    ...
    def conversations_list(
        limit: int, page: int, business_id: Optional[int] = None
    ) -> str:
        """Chave para lista de conversas"""
        return f"conversations:list:{business_id or 'all'}:{limit}:{page}"

    @staticmethod
    def dashboard_stats(business_id: int, period: str = "daily") -> str:

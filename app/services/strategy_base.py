...
class BaseStrategy(ABC):
    ...
    def update_metrics(self, processing_time: float, success: bool):
        ...
        self.performance_metrics["success_rate"] = (
            self.performance_metrics["successes"] / self.performance_metrics["requests"]
        )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {"name": self.name, **self.performance_metrics}
...
class StrategySelector:
    ...
    def select_strategy(
        self, context: MessageContext, available_strategies: List[BaseStrategy]
    ) -> BaseStrategy:
        ...
        for rule in self.rules:
            ...
            if strategy_type:
                ...
                for strategy in available_strategies:
                    if (
                        strategy.name.lower() == strategy_type.value
                        and strategy.can_handle(context)
                    ):
                        return strategy
    ...
    def _check_high_value_customer(
    ...
    def _check_simple_query(self, context: MessageContext) -> Optional[StrategyType]:
        ...
        if word_count <= 3 and context.lead_score < 40 and context.complexity == "low":
            return StrategyType.ADVANCED
        return None

    def _check_sales_opportunity(
        self, context: MessageContext
    ) -> Optional[StrategyType]:

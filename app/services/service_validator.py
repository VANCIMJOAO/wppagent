...
class ServiceValidationService:
    ...
    async def validate_and_get_service(
        ...
        try:
            ...
            if (
                similar_matches
                and similar_matches[0].confidence >= self.similarity_threshold
            ):
    ...
    async def verify_service_capacity(
        self, db: AsyncSession, service_id: int, date_range: Tuple[str, str]
    ) -> Dict:
        """
        ...
        try:
            ...
            appointments_query = await db.execute(
                select(
                    func.count(Appointment.id), func.avg(Appointment.price_at_booking)
                ).where(
                    and_(
            ...
            appointments_per_day = (
                appointment_count / days_in_period if days_in_period > 0 else 0
            )

            # Estimar capacidade máxima (assumindo 8 horas úteis por dia)
            daily_capacity = (
                (8 * 60) // service.duration_minutes if service.duration_minutes else 8
            )
            total_capacity = daily_capacity * days_in_period
            utilization_rate = (
                (appointment_count / total_capacity * 100) if total_capacity > 0 else 0
            )
    ...
    async def _find_exact_match(
        self, db: AsyncSession, service_input: str
    ) -> Optional[Service]:
        ...
        try:
            ...
            for service in all_services:
                ...
                name_similarity = SequenceMatcher(
                    None, input_lower, service.name.lower()
                ).ratio()
                ...
                if common_words:
                    keyword_bonus = (
                        len(common_words)
                        / max(len(input_words), len(service_words))
                        * 0.3
                    )
                ...
                final_confidence = min(
                    1.0, name_similarity + keyword_bonus + category_bonus
                )
    ...
    async def _auto_create_service(
        self, db: AsyncSession, service_name: str, business_id: Optional[int] = None
    ) -> Optional[Service]:
        ...
        try:
            ...
            if not business_id:
                ...
                if not business:
                    business = Business(name="Negócio Principal", created_at=func.now())
                    db.add(business)
                    await db.flush()
            ...
            logger.info(
                f"Serviço criado automaticamente: {new_service.name} (ID: {new_service.id})"
            )
            return new_service
    ...
    async def _get_related_services(
        self, db: AsyncSession, user_history: List[str]
    ) -> List[ServiceSuggestion]:
        ...
        try:
            ...
            for service_name in user_history:
                ...
                category_services = await db.execute(
                    select(Service).where(
                        and_(Service.is_active == True, Service.name != service_name)
                    )
                )

...
from ..models.database import AdminUser, PushNotification, PushSubscription
...
class PushNotificationService:
    ...
    async def unsubscribe_admin(
        self, db: Session, admin_user_id: int, endpoint: Optional[str] = None
    ) -> int:
        """
    ...
    async def send_notification(
        ...
        if not PYWEBPUSH_AVAILABLE:
            logger.warning(
                "🚫 Push notification request ignored - pywebpush not available"
            )
            logger.info(
                f"📄 Would send: '{title}' - '{body}' to {subscription.endpoint}"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"Push notification enviada: {title} -> {subscription.endpoint[:50]}..."
            )
            return True
    ...
    async def send_to_admin(
        self, db: Session, admin_user_id: int, title: str, body: str, **kwargs
    ) -> int:
        """
        ...
        try:
            ...
            # Enviar em paralelo
            for subscription in subscriptions:
                task = self.send_notification(db, subscription, title, body, **kwargs)
                tasks.append(task)
            ...
            logger.info(
                f"Enviadas {success_count}/{len(subscriptions)} notificações para admin {admin_user_id}"
            )
            return success_count

        except Exception as e:
            logger.error(
                f"Erro ao enviar notificações para admin {admin_user_id}: {str(e)}"
            )
            return 0

    async def send_to_all_admins(
        self, db: Session, title: str, body: str, **kwargs
    ) -> int:
        """
        ...
        try:
            ...
            logger.info(
                f"Enviadas {success_count}/{len(subscriptions)} notificações para todos os admins"
            )
            return success_count
    ...
    async def get_admin_subscriptions(
        self, db: Session, admin_user_id: int
    ...
    async def get_notification_stats(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            notifications_24h = (
                ...
                .filter(PushNotification.sent_at >= last_24h)
                .count()
            )
            ...
            notifications_7d = (
                db.query(PushNotification)
                .filter(PushNotification.sent_at >= last_7d)
                .count()
            )
            ...
            success_rate = (
                (success_24h / notifications_24h * 100) if notifications_24h > 0 else 0
            )
            ...
            top_admins = (
                ...
                .join(PushSubscription, AdminUser.id == PushSubscription.admin_user_id)
                .join(
                    PushNotification,
                    PushSubscription.id == PushNotification.subscription_id,
                )
                .filter(PushNotification.sent_at >= last_7d)
                .group_by(AdminUser.id, AdminUser.username)
                .order_by(desc("notification_count"))
                .limit(5)
                .all()
            )

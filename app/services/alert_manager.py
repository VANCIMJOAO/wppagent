...
class AlertManager:
    ...
    def __init__(self):
        ...
        self.email_config = {
            "enabled": (
                ...
                if hasattr(settings, "alert_email_enabled")
                else False
            ),
            ...
            "from_email": getattr(
                settings, "alert_from_email", "whatsapp-agent@localhost"
            ),
            "to_emails": (
                getattr(settings, "alert_to_emails", "").split(",")
                if hasattr(settings, "alert_to_emails")
                else []
            ),
        }
    ...
    async def _send_email_notification(self, alert: Alert):
        ...
        try:
            ...
            logger.info(
                f"Email de alerta enviado para: {', '.join(self.email_config['to_emails'])}"
            )
    ...
    def get_alert_history(self, limit: int = 100) -> List[Alert]:
        ...
        return sorted(
            self.alert_history[-limit:], key=lambda x: x.timestamp, reverse=True
        )
    ...
    def get_alert_stats(self) -> Dict[str, Any]:
        ...
        for alert in active_alerts:
            ...
            stats["alerts_by_severity"][severity] = (
                stats["alerts_by_severity"].get(severity, 0) + 1
            )

        # Contar por fonte
        for alert in active_alerts:
            source = alert.source
            stats["alerts_by_source"][source] = (
                stats["alerts_by_source"].get(source, 0) + 1
            )

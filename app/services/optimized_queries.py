...
class OptimizedQueries:
    ...
    async def get_appointments_with_full_details(
        ...
        """
        logger.info(
            f"Fetching optimized appointments - user_id: {user_id}, business_id: {business_id}"
        )
        ...
        logger.info(
            f"Retrieved {len(appointments_dict)} appointments with full details"
        )
    ...
    async def get_conversations_with_messages_count(
        ...
        """
        logger.info(
            f"Fetching optimized conversations - user_id: {user_id}, business_id: {business_id}"
        )
        ...
        query = (
            select(
                ...
                func.count(case((Message.is_from_user == True, 1))).label(
                    "user_messages_count"
                ),
                func.count(case((Message.is_from_user == False, 1))).label(
                    "business_messages_count"
                ),
            )
    ...
    async def get_businesses_with_service_stats(
        ...
        query = (
            select(
                ...
                func.count(case((Appointment.status == "scheduled", 1))).label(
                    "scheduled_appointments"
                ),
                func.count(case((Appointment.status == "completed", 1))).label(
            ...
            businesses.append(
                {
                    ...
                    "service_stats": {
                        ...
                        "avg_service_price": (
                            float(row.avg_service_price)
                            if row.avg_service_price
                            else 0.0
                        ),
                        "min_service_price": (
                            float(row.min_service_price)
                            if row.min_service_price
                            else 0.0
                        ),
                        "max_service_price": (
                            float(row.max_service_price)
                            if row.max_service_price
                            else 0.0
                        ),
    ...
    async def get_dashboard_analytics_optimized(
        ...
        """
        logger.info(
            f"Fetching optimized dashboard analytics - business_id: {business_id}"
        )
        ...
        appointment_query = (
            select(
                ...
                func.count(case((Appointment.status == "cancelled", 1))).label(
                ...
                func.count(func.distinct(Appointment.business_id)).label(
                    "unique_businesses"
                ),
            )
            .join(Service, Appointment.service_id == Service.id)
        ...
        conversation_query = (
            select(
                ...
                func.count(case((Conversation.status == "active", 1))).label(
        ...
        analytics = {
            ...
            "appointments": {
                ...
                "avg_duration_minutes": (
                    float(appointment_row.avg_duration)
                    if appointment_row.avg_duration
                    else 0.0
                ),
                "completion_rate": (appointment_row.completed_appointments or 0)
                / max(appointment_row.total_appointments or 1, 1)
                * 100,
            },
            "revenue": {
                "total": (
                    float(appointment_row.total_revenue)
                    if appointment_row.total_revenue
                    else 0.0
                ),
                "avg_per_appointment": float(appointment_row.total_revenue or 0)
                / max(appointment_row.total_appointments or 1, 1),
            },
            "clients": {
                "total_unique": appointment_row.unique_clients or 0,
                "avg_appointments_per_client": (appointment_row.total_appointments or 0)
                / max(appointment_row.unique_clients or 1, 1),
            },
            "conversations": {
            ...
            "businesses": {"total_serving": appointment_row.unique_businesses or 0},
        }

        logger.info(
            f"Generated optimized dashboard analytics with {analytics['appointments']['total']} appointments"
        )
        return analytics

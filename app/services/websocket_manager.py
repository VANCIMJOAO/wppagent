...
class WebSocketConnection:
    def __init__(self, websocket: WebSocket, user_id: str, subscriptions: Set[str] = None):
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, subscriptions: List[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ WebSocket connected: {user_id} with subscriptions: {connection.subscriptions}")
            
            # Send initial connection success message
            await connection.send_event(WebSocketEventType.CONNECTION_STATUS, {
                "connected": True,
                "user_id": user_id,
    ...
    async def broadcast_to_topic(self, topic: str, event_type: WebSocketEventType, data: dict, exclude_user: str = None) -> int:
        ...
        if successful_sends > 0:
            logger.info(f"📡 Broadcasted {event_type.value} to {successful_sends} clients on topic: {topic}")
        
        return successful_sends
    
    async def send_to_user(self, user_id: str, event_type: WebSocketEventType, data: dict) -> bool:
    ...
    async def broadcast_to_all(self, event_type: WebSocketEventType, data: dict) -> int:
        ...
        if successful_sends > 0:
            logger.info(f"📡 Broadcasted {event_type.value} to {successful_sends} clients")
        ...
        if message_type == "heartbeat":
            ...
            await connection.send_event(
                WebSocketEventType.HEARTBEAT_RESPONSE,
                {
                    ...
                    "server_time": datetime.utcnow().isoformat(),
                    "connection_duration": int((datetime.utcnow() - connection.connected_at).total_seconds())
                }
            )
            ...
            await connection.send_event(
                WebSocketEventType.CONNECTION_STATUS,
                {
                    ...
                    "history_count": len(recent_events)
                }
            )
    ...
    def _get_recent_events_for_user(self, subscriptions: Set[str], minutes: int = 5) -> List[Dict]:
        """Get recent events relevant to user's subscriptions"""
        cutoff_time = datetime.utcnow().replace(microsecond=0) - timedelta(minutes=minutes)
    ...
    def get_connection_stats(self) -> dict:
        ...
        for topic, users in self.topic_subscriptions.items():
            active_users = [uid for uid in users if uid in self.connections and self.connections[uid].is_alive]
        ...
        return {
            ...
            "active_users": [uid for uid, conn in self.connections.items() if conn.is_alive],

...
class BasicAlertManager:
    ...
    async def evaluate_alerts(self, metrics: Dict[str, float]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            ...
            metadata={"metrics_count": len(metrics), "rules_count": len(self.alert_rules)},
            ...
            try:
                if await self._should_fire_alert(alert_name, rule, metrics):
                    ...
                    alert = {
                        ...
                        "recommended_action": rule.get("recommended_action", "Verificar logs do sistema"),
    ...
    async def _should_fire_alert(self, alert_name: str, rule: Dict, metrics: Dict[str, float]) -> bool:
...
async def get_active_alerts(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.info(
            ...
            metadata={"total_alerts": len(active_alerts), "status": alert_summary["status"]},
        ...
        return {
            "alerts": [],
            "summary": {"status": "error", "message": f"Erro ao avaliar alertas: {str(e)}"},
            "total_active": 0,
            "evaluated_at": datetime.utcnow().isoformat(),
            "error": str(e)
        }
...
async def _convert_health_to_metrics(health_data: Dict[str, Any]) -> Dict[str, float]:
    ...
    metrics = {
        # Webhook metrics
        "webhook_failure_rate": components.get("webhook", {}).get("blocked_percentage", 0),
        "avg_processing_time": components.get("webhook", {}).get("response_time_ms", 0),
        
        # Database metrics  
        "db_connection_errors": 1 if components.get("database", {}).get("status") != "healthy" else 0,
        
        # Redis metrics
        "redis_health": 1 if components.get("redis", {}).get("status") == "healthy" else 0,
        
        # Meta API metrics
        "meta_api_errors": 1 if components.get("meta_api", {}).get("status") != "healthy" else 0,
        
        # Business metrics
        "appointments_per_hour": components.get("database", {}).get("appointments_count", 0) / 24,  # Estimativa
        
        # Performance metrics
        "total_response_time": health_data.get("performance", {}).get("total_check_time_ms", 0)
    }
...
async def get_alert_summary(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "critical_count": alert_response["summary"]["by_severity"].get("critical", 0),
        ...
        logger.error(f"❌ OM003 Erro no resumo de alertas: {str(e)}", category="alerts_summary")

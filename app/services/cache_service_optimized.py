...
class OptimizedCacheService:
    ...
    async def set(
        self, key: str, data: Any, cache_type: CacheType, ttl: int = None
    ) -> bool:
        """
        ...
        try:
            ...
            if self.redis:
                ...
                self.memory_cache_timestamps[cache_key] = datetime.now() + timedelta(
                    seconds=ttl
                )
            ...
            self.metrics["sets"] += 1
            logger.debug(
                f"📝 Cache SET: {cache_key} (TTL: {ttl}s, Type: {cache_type.value})"
            )
            return True
            ...
            if self.redis:
                ...
                if "*" in cache_key:
                    ...
                    if keys:
                        ...
                        logger.debug(
                            f"🗑️ Redis Cache DELETE: {len(keys)} chaves removidas"
                        )
                        return True
                else:
                ...
                if "*" in cache_key:
                    ...
                    logger.debug(
                        f"🗑️ Memory Cache DELETE: {len(keys_to_remove)} chaves removidas"
                    )
    ...
    async def cache_user_context(
        self, wa_id: str, context: Dict[str, Any], ttl: int = 900
    ):
        """Cache otimizado do contexto do usuário"""
        return await self.set(
            f"user_context_{wa_id}", context, CacheType.USER_CONTEXT, ttl
        )
    ...
    async def cache_intent_analysis(
        self, message: str, intent_data: Dict[str, Any], ttl: int = 1800
    ):
        """Cache de análise de intenção com hash da mensagem"""
        message_hash = hashlib.md5(message.lower().strip().encode()).hexdigest()
        return await self.set(
            f"intent_{message_hash}", intent_data, CacheType.INTENT, ttl
        )
    ...
    async def cache_response(self, context_key: str, response: str, ttl: int = 3600):
        ...
        return await self.set(
            f"response_{context_key}", response, CacheType.RESPONSE, ttl
        )
    ...
    async def cache_query_result(self, query_hash: str, result: Any, ttl: int = 600):
        ...
        return await self.set(
            f"query_{query_hash}", result, CacheType.QUERY_RESULT, ttl
        )
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if self.metrics["compression_savings"] > 0:
            ...
            stats["compression_savings_mb"] = round(
                self.metrics["compression_savings"] / 1024 / 1024, 2
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                self.metrics["errors"] > self.metrics["sets"] * 0.1
            ):  # Mais de 10% de erro
    ...
    async def cleanup_expired_memory_cache(self):
        ...
        now = datetime.now()
        expired_keys = [
            key for key, expiry in self.memory_cache_timestamps.items() if now >= expiry
        ]
        ...
        if expired_keys:
            logger.debug(
                f"🧹 Limpeza cache memória: {len(expired_keys)} entradas expiradas removidas"
            )

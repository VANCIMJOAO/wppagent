...
class UserService:
    ...
    async def get_or_create_user(
        db: AsyncSession, wa_id: str, nome: str = None, telefone: str = None
    ) -> User:
        """
        ...
        if not user:
            # Criar novo usuário
            user = User(wa_id=wa_id, nome=nome, telefone=telefone)
            db.add(user)
            await db.commit()
...
class ConversationService:
    ...
    async def update_conversation_status(
        db: AsyncSession, conversation_id: int, status: str
    ) -> Conversation:
    ...
    @staticmethod
    async def get_conversations_for_dashboard(
        ...
        for conversation, user in result.all():
            ...
            unread_count = len(
                [m for m in conversation.messages if m.direction == "in"]
            )
...
class AppointmentService:
    ...
    async def create_appointment(
        ...
        try:
            ...
            if not business:
                ...
                business = Business(name="Negócio Principal", created_at=datetime.now())
                db.add(business)
                await db.flush()
            ...
            if not date_time and appointment_date:
                try:
                    ...
                    if not date_time:
                        ...
                        logger.warning(
                            f"Não foi possível parsear data '{appointment_date}', usando fallback"
                        )
            ...
            if service_id:
                ...
                if service_obj and service_obj.duration_minutes:
                    ...
                    end_time = date_time + timedelta(
                        minutes=service_obj.duration_minutes
                    )
            ...
            logger.info(
                f"Agendamento criado: ID={appointment.id}, User={user_id}, Service={service_id}, DateTime={date_time}"
            )
            return appointment
    ...
    async def update_appointment_status(
        db: AsyncSession, appointment_id: int, status: str
    ...
    async def confirm_appointment(
        db: AsyncSession, appointment_id: int, confirmed_by: str = "customer"
    ) -> Optional[Appointment]:
        """
        ...
        try:
            ...
            if appointment:
                ...
                await db.refresh(appointment)
                logger.info(
                    f"Agendamento {appointment_id} confirmado por {confirmed_by}"
                )
                return appointment
            else:
                logger.warning(
                    f"Agendamento {appointment_id} não encontrado para confirmação"
                )
                return None
    ...
    async def cancel_appointment(
        ...
        try:
            ...
            if appointment:
                ...
                await db.refresh(appointment)
                logger.info(
                    f"Agendamento {appointment_id} cancelado por {cancelled_by}: {reason}"
                )
                return appointment
            else:
                logger.warning(
                    f"Agendamento {appointment_id} não encontrado para cancelamento"
                )
                return None
    ...
    async def reschedule_appointment(
        db: AsyncSession, appointment_id: int, new_date_time: datetime
    ) -> Optional[Appointment]:
        """
        ...
        try:
            ...
            if appointment_service:
                ...
                new_end_time = new_date_time + timedelta(
                ...
                await db.refresh(appointment)
                logger.info(
                    f"Agendamento {appointment_id} reagendado para {new_date_time}"
                )
                return appointment
            else:
                logger.warning(
                    f"Agendamento {appointment_id} não encontrado para reagendamento"
                )
                return None
...
class MetricsService:
    ...
    async def get_dashboard_metrics(db: AsyncSession) -> Dict:
        ...
        pending_appointments = await db.scalar(
            select(func.count(Appointment.id)).where(Appointment.status == "pendente")
        )

        # Conversas que precisam de atenção humana
        human_conversations = await db.scalar(
            select(func.count(Conversation.id)).where(Conversation.status == "human")
        )

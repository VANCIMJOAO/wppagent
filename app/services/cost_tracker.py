...
class OpenAICostTracker:
    ...
    def track_usage(
        ...
        try:
            ...
            if model not in self.monthly_usage[month]:
                self.monthly_usage[month][model] = {
                    "input": 0,
                    "output": 0,
                    "calls": 0,
                    "minutes": 0,
            ...
            logger.info(
                f"📊 Uso registrado: {model} - Tokens: {input_tokens + output_tokens}"
            )
    ...
    def _check_usage_limits(self):
        ...
        if daily_cost >= self.daily_limit_usd:
            logger.warning(
                f"🚨 LIMITE DIÁRIO ATINGIDO: ${daily_cost:.4f} / ${self.daily_limit_usd}"
            )
        elif daily_cost >= self.daily_limit_usd * self.alert_threshold:
            logger.warning(
                f"⚠️ Alerta diário: ${daily_cost:.4f} / ${self.daily_limit_usd} ({daily_cost/self.daily_limit_usd*100:.1f}%)"
            )

        # Alertas mensais
        if monthly_cost >= self.monthly_limit_usd:
            logger.warning(
                f"🚨 LIMITE MENSAL ATINGIDO: ${monthly_cost:.4f} / ${self.monthly_limit_usd}"
            )
        elif monthly_cost >= self.monthly_limit_usd * self.alert_threshold:
            logger.warning(
                f"⚠️ Alerta mensal: ${monthly_cost:.4f} / ${self.monthly_limit_usd} ({monthly_cost/self.monthly_limit_usd*100:.1f}%)"
            )
    ...
    def get_detailed_report(self) -> Dict:
        ...
        return {
            ...
            "daily": {
                ...
                "usage_percentage": (
                    (daily_cost / self.daily_limit_usd * 100)
            ...
            "monthly": {
                ...
                "usage_percentage": (
                    (monthly_cost / self.monthly_limit_usd * 100)
    ...
    def get_cost_projection(self, days_ahead: int = 30) -> Dict:
        ...
        try:
            ...
            return {
                ...
                f"projected_{days_ahead}days_brl": projected_custom
                * self.exchange_rate,
                ...
                "risk_level": (
                    ...
                    if projected_monthly > self.monthly_limit_usd
                        ...
                        "medium"
                        if projected_monthly > self.monthly_limit_usd * 0.8
                        else "low"
                    )
                ),
            }

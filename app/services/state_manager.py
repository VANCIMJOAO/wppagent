...
class BookingContext:
    ...
    step: str = (
        "initial"  # initial, service_selection, datetime, confirmation, completed
    )
    attempts: int = 0
...
class ConversationState:
    ...
    def to_json(self) -> str:
        ...
        data = {
            ...
            "booking_context": (
                self.booking_context.to_dict() if self.booking_context else None
            ),
...
class ConversationStateManager:
    ...
    def __init__(self, redis_url: str = None, default_ttl: int = 1800):
        ...
        if not redis_url:
            try:
                ...
                logger.info(
                    f"🔗 Using configured Redis: {redis_url.split('@')[-1]}"
                )  # Log sem credenciais
        ...
        # Estatísticas
        self.stats = {"loads": 0, "saves": 0, "errors": 0, "cleanups": 0}
    ...
    async def save_state(
        self, user_id: str, state: ConversationState, ttl: Optional[int] = None
    ):
        """
    ...
    async def get_active_conversations(self) -> List[str]:
        ...
        try:
            ...
            for user_id, state in self.memory_cache.items():
                if not state.is_expired() and state.status in [
                    ConversationStatus.ACTIVE,
                    ConversationStatus.BOOKING_FLOW,
                ]:
...
async def add_user_message(
    user_id: str, content: str, role: MessageRole = MessageRole.USER, **metadata
):

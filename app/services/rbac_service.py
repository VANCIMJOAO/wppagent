...
from app.models.rbac import (
    RBACUser, RBACRole, RBACPermission,
    ...
    async def _create_default_permissions(self, session: AsyncSession):
        ...
        for perm_type, definition in PERMISSION_DEFINITIONS.items():
            ...
            existing = await session.execute(
                select(RBACPermission).where(RBACPermission.permission_type == perm_type)
            )
        ...
        self.logger.info(f"✅ {len(PERMISSION_DEFINITIONS)} permissões criadas/verificadas")
    ...
    async def get_user_by_id(self, user_id: int) -> Optional[RBACUser]:
        ...
        async with AsyncSessionLocal() as session:
            result = await session.execute(
                select(RBACUser)
                .options(selectinload(RBACUser.roles).selectinload(RBACRole.permissions))
        ...
        async with AsyncSessionLocal() as session:
            ...
            return {
                ...
                "permissions": {
                    ...
                    "categories": len(set(def_.category for def_ in PERMISSION_DEFINITIONS.values()))
                }
            }

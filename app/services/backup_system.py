        ...
        try:
            ...
            if result.returncode == 0:
                ...
                checksum = await loop.run_in_executor(
                    self.executor, self._calculate_checksum, str(final_path)
                )
        ...
        try:
            ...
            for backup in backups_to_remove:
                try:
                    ...
                    business_logger.error(
                        f"Error removing backup {backup.filename}: {e}"
                    )
    ...
    async def verify_backup(self, backup_info: BackupInfo) -> bool:
        ...
        try:
            ...
            current_checksum = await asyncio.get_event_loop().run_in_executor(
                self.executor, self._calculate_checksum, backup_info.filepath
            )
    ...
    async def restore_backup(self, backup_info: BackupInfo) -> bool:
        ...
        try:
            business_logger.warning(
                f"Starting database restore from {backup_info.filename}"
            )
            ...
            if backup_info.compressed:
                temp_file = str(
                    self.backup_dir / f"temp_restore_{int(time.time())}.sql"
                )
            ...
            if result.returncode == 0:
                business_logger.info(
                    f"Database restore completed successfully from {backup_info.filename}"
                )
                return True
            else:
                error_message = result.stderr or "Unknown error"
    ...
    def get_backup_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "success_rate": len(successful_backups)
                / len(self.backup_history)
                * 100,
                "last_backup": (
                    self.last_backup_status.to_dict()
                    if self.last_backup_status
                    else None
                ),
                "total_size_mb": sum(b.size_bytes for b in successful_backups)
                / (1024**2),
                "average_duration_minutes": (
                    sum(b.duration_seconds for b in successful_backups)
                ...
                "oldest_backup": (
                    min(
                        self.backup_history, key=lambda x: x.timestamp
                    ).timestamp.isoformat()
            ...
            for backup in recent_backups:
                if backup.success:
                    ...
                    if is_valid:
                        ...
                        failed_count += 1
                        business_logger.error(
                            f"Backup verification failed: {backup.filename}"
                        )

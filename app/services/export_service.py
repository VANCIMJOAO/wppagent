...
class ReportExportService:
    ...
    async def export_appointments_csv(
        ...
        try:
            ...
            query = (
                ...
                .select_from(
                    Appointment.join(User, Appointment.user_id == User.id)
                ...
                data.append(
                    {
                        ...
                        "Valor": (
                            f"R$ {float(row.price):.2f}" if row.price else "R$ 0,00"
                        ),
    ...
    async def export_analytics_excel(self, period_days: int = 30) -> BytesIO:
        ...
        try:
            ...
            funnel_data = await self.analytics.get_conversion_funnel(
                start_date, end_date
            )
            ...
            currency_format = workbook.add_format(
                {"num_format": "R$ #,##0.00", "align": "center", "border": 1}
            )
            ...
            summary_metrics = [
                (
                    ...
                    f"{funnel_data['conversion_rates']['overall_conversion']:.1f}%",
                ),
                (
                    "Total Primeiros Contatos",
                    funnel_data["funnel_stages"]["first_contact"],
                ),
                ("Total Agendamentos", funnel_data["funnel_stages"]["scheduled"]),
                ("Agendamentos Realizados", funnel_data["funnel_stages"]["completed"]),
                (
                    "Receita Total",
                    f"R$ {business_data['revenue']['current_period']:.2f}",
                ),
            ...
            funnel_rows = [
                ...
                (
                    "Agendado",
                    funnel_data["funnel_stages"]["scheduled"],
                    funnel_data["conversion_rates"]["contact_to_schedule"] / 100,
                ),
                (
                    "Confirmado",
                    funnel_data["funnel_stages"]["confirmed"],
                    funnel_data["conversion_rates"]["schedule_to_confirm"] / 100,
                ),
                (
                    "Realizado",
                    funnel_data["funnel_stages"]["completed"],
    ...
    async def export_executive_pdf(self, period_days: int = 30) -> BytesIO:
        ...
        try:
            ...
            story.append(
                Paragraph("WhatsApp Agent - Business Intelligence", styles["Heading2"])
            )
            ...
            summary_data = [
                ...
                [
                    "Total de Primeiros Contatos",
                    f"{funnel_data['funnel_stages']['first_contact']:,}",
                ],
                [
                    "Agendamentos Realizados",
                    f"{funnel_data['funnel_stages']['completed']:,}",
                ],
                [
                ...
                [
                    "Clientes VIP Ativos",
                    f"{customer_data['customer_summary']['total_vip']:,}",
                ],
                [
                    "Taxa de Confirmação",
            ...
            funnel_table_data = [
                ...
                [
                    "Resposta do Bot",
                    f"{funnel_data['funnel_stages']['bot_response']:,}",
                    "-",
                    "-",
                ],
                [
                    "Agendamentos Criados",
                    f"{funnel_data['funnel_stages']['scheduled']:,}",
                ...
                [
                    ...
                    f"{funnel_data['funnel_stages']['confirmed']:,}",
                    f"{funnel_data['conversion_rates']['schedule_to_confirm']:.1f}%",
            ...
            funnel_table = Table(
                funnel_table_data,
                colWidths=[2.2 * inch, 1.2 * inch, 1.2 * inch, 1 * inch],
            )
            ...
            if customer_data["vip_customers"]:
                ...
                vip_table_data = [
                    ["CLIENTE", "TELEFONE", "AGENDAMENTOS", "VALOR TOTAL"]
                ]

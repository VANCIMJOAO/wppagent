...
class RealtimeWebSocketManager:
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            "messages_failed": 0,
            "rooms_active": 0,
            "uptime_started": datetime.utcnow(),
    ...
    async def connect(
        ...
        try:
            ...
            logger.info(f"🔌 Conexão WebSocket estabelecida: {user_id} ({connection_id}) -> {subscriptions}")
    ...
    async def disconnect(self, connection_id: str, reason: str = "Client disconnected"):
        ...
        try:
            ...
            logger.info(f"🔌 Conexão WebSocket desconectada: {connection.user_id} ({connection_id}) - {reason}")
    ...
    async def _join_room(self, connection_id: str, room_id: str):
        ...
        # Update stats
        self.stats["rooms_active"] = len([r for r in self.rooms.values() if r.connections])
    ...
    async def broadcast_to_room(
        ...
        if tasks:
            ...
            logger.info(f"📡 Broadcast para sala '{room_id}': {successful_sends}/{len(tasks)} conexões")
    ...
    async def broadcast_to_subscriptions(
        ...
        return await self.broadcast_to_room(room_id, event_type, data, priority=priority)
    ...
    async def broadcast_to_user(
        ...
        if tasks:
            ...
            logger.info(f"📡 Broadcast para usuário '{user_id}': {successful_sends}/{len(tasks)} conexões")
            return successful_sends
    ...
    async def handle_client_message(self, connection_id: str, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"❌ Erro ao processar mensagem do cliente {connection_id}: {e}")
        ...
        while self._running:
            try:
                ...
                if stale_connections:
                    logger.info(f"🔄 Limpeza de heartbeat: {len(stale_connections)} conexões removidas")
        ...
        while self._running:
            try:
                ...
                for room_id, room in self.rooms.items():
                    if not room.connections and now - room.last_activity > timedelta(hours=1):
                ...
                if old_rooms:
                    logger.info(f"🧹 Limpeza de salas: {len(old_rooms)} salas vazias removidas")

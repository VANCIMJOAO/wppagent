...
class ConversationFlowEngine:
        ...
        async with self.processing_locks[user_id]:
            ...
            if user_id in self.response_control:
                ...
                if time_diff < 5:
                    ...
                    if self._is_similar_message(
                        message, last_response.get("message", "")
                    ):
                        logger.info(
                            f"🔄 Ignorando mensagem similar para {user_id}: {message[:50]}..."
                        )
                        return False
    ...
    def analyze_conversation_flow(
        ...
        confidence = self._calculate_decision_confidence(
            detected_topics, memory, transition_type
        )

        # 7. Gerar reasoning
        reasoning = self._generate_reasoning(
            detected_topics, memory, transition_type, next_state
        )
        ...
        return FlowDecision(
            ...
            context_to_maintain=self._identify_context_to_maintain(
                memory, detected_topics
            ),
        ...
        for topic in detected_topics:
            if topic.topic_id in memory.active_topics:
                ...
                memory.active_topics[topic.topic_id].confidence = max(
                    memory.active_topics[topic.topic_id].confidence, topic.confidence
                )
            else:
    ...
    def _generate_agent_instructions(
        ...
        if transition_type == FlowTransition.BACK_REFERENCE:
            instructions["context_awareness"].append(
                "Client is referring back to previous topic"
            )
            instructions["response_style"] = "acknowledging_reference"
            instructions["priority_actions"].append(
                "Connect current response to previous context"
            )
            ...
            instructions["priority_actions"].append(
                "Acknowledge topic change gracefully"
            )

        elif transition_type == FlowTransition.INTERRUPT:
            instructions["context_awareness"].append(
                "Client is interrupting/interjecting"
            )
            ...
            instructions["priority_actions"].append(
                "Address all topics in order of priority"
            )
        ...
        for topic_id, topic in memory.active_topics.items():
            if not topic.resolved and topic.mentions > 0:
                active_topics_context.append(
                    f"{topic.name} (mentioned {topic.mentions}x)"
                )
    ...
    def _calculate_decision_confidence(
        ...
        if detected_topics:
            topic_confidence = sum(t.confidence for t in detected_topics) / len(
                detected_topics
            )
    ...
    def _generate_reasoning(
        ...
        if detected_topics:
            ...
            reasoning_parts.append(
                f"Primary topic detected: {primary_topic.name} (confidence: {primary_topic.confidence:.2f})"
            )
        ...
        reasoning_parts.append(
            transition_reasons.get(transition_type, "Standard progression")
        )

        # Reasoning sobre estado atual
        reasoning_parts.append(
            f"Current state: {memory.current_state.value} → Next state: {next_state.value}"
        )

        # Contexto adicional
        if memory.context_switches > 3:
            reasoning_parts.append(
                "High context switching detected - maintaining flexibility"
            )
    ...
    def _generate_contextual_response(
        ...
        if next_state == ConversationState.MULTI_INTENT and len(detected_topics) > 1:
            ...
            if transition_type == FlowTransition.MULTI_TOPIC:
                topics_list = [t.name for t in detected_topics]
                return transition_templates[transition_type].format(
    ...
    def _identify_context_to_maintain(
        self, memory: ConversationMemory, detected_topics: List[ConversationTopic]
    ) -> List[str]:
...
class ConversationMemoryManager:
    ...
    async def remember_service_discussion(
        self, user_id: str, service_name: str, details: Dict[str, Any]
    ):
        ...
        self.service_discussions[user_id][service_name] = {
            ...
            "details": details,
            "mentions": self.service_discussions[user_id]
            .get(service_name, {})
            .get("mentions", 0)
            + 1,
        }
    ...
    async def remember_scheduling_context(
        self, user_id: str, service_name: str, scheduling_data: Dict[str, Any]
    ):
    ...
    async def get_relevant_context(
        ...
        if user_id in self.scheduling_contexts:
            ...
            if scheduling_context["last_updated"] > datetime.now() - timedelta(
                minutes=15
            ):
        ...
        if recent_history:
            relevant_info["recent_conversation"] = recent_history[
                -3:
            ]  # Últimas 3 mensagens
...
class ResponseMonitor:
    ...
    async def log_response(
        self, user_id: str, message: str, response: str, metadata: Dict[str, Any] = None
    ):
    ...
    async def _detect_multiple_responses(
        self, user_id: str, message: str, response: str
    ) -> bool:
        ...
        recent_logs = [
            ...
            and log["timestamp"] > datetime.now() - timedelta(seconds=30)
        ]

        if len(recent_logs) > self.alert_thresholds["multiple_responses"]:
            logger.warning(
                f"🚨 Múltiplas respostas detectadas para {user_id}: {len(recent_logs)} respostas"
            )
            return True
    ...
    async def _detect_context_loss(
        ...
        if len(recent_logs) >= 2:
            ...
            if similarity < self.alert_thresholds["context_loss_threshold"]:
                logger.warning(
                    f"🚨 Perda de contexto detectada para {user_id}: similaridade {similarity:.2f}"
                )
                return True

        return False

    async def _detect_inconsistent_responses(
        self, user_id: str, message: str, response: str
    ) -> bool:
    ...
    async def _detect_spam_patterns(
        self, user_id: str, message: str, response: str
    ) -> bool:
        ...
        recent_logs = [
            ...
            for log in self.response_logs
            if log["user_id"] == user_id
            and log["timestamp"] > datetime.now() - timedelta(minutes=1)
        ]

        if len(recent_logs) > 5:  # Mais de 5 mensagens por minuto
            logger.warning(
                f"🚨 Padrão de spam detectado para {user_id}: {len(recent_logs)} mensagens/min"
            )
            return True
    ...
    async def _handle_anomaly(
        self, anomaly_type: str, user_id: str, message: str, response: str
    ):
    ...
    async def _cleanup_old_logs(self, max_age_hours: int = 24):
        ...
        cutoff_time = datetime.now() - timedelta(hours=max_age_hours)
        self.response_logs = [
            log for log in self.response_logs if log["timestamp"] > cutoff_time
        ]
        ...
        return {
            ...
            "anomalies_detected": len(
                [
                    log
                    for log in self.response_logs
                    if "anomaly" in log.get("metadata", {})
                ]
            ),
            "active_users": len(
                set(log["user_id"] for log in self.response_logs[-100:])
...
class ConversationFlowService:
    ...
    async def process_message(
        self, message: str, user_phone: str, context: Dict[str, Any] = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            should_process = await self.flow_engine.ensure_single_response(
                user_phone, message
            )
            ...
            # 3. ROTEAR MENSAGEM
            routing_result = await self.response_router.route_message(
                message, relevant_context
            )

            # 4. ATUALIZAR MEMÓRIA DA CONVERSAÇÃO
            await self.memory_manager._update_conversation_memory(
            ...
            await self.response_monitor.log_response(
                ...
                {
                    "routing_intent": routing_result["intent"],
                    "confidence": routing_result["confidence"],
                },
            )

            # 6. MARCAR RESPOSTA COMO ENVIADA
            await self.flow_engine.mark_response_sent(
                user_phone, message, routing_result["response"]
            )

            logger.info(
                f"✅ Resposta única processada para {user_phone}: {routing_result['response_type']}"
            )
        ...
        try:
            ...
            try:
                ...
                response = asyncio.run(
                    self.process_message(message, user_phone, context)
                )
            except RuntimeError:
    ...
    def get_conversation_summary(self, user_phone: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "previous_state": (
                memory.previous_state.value if memory.previous_state else None
            ),
            "active_topics": {
                topic_id: {
                    "name": topic.name,
            ...
            "conversation_duration": str(
                memory.last_activity - memory.conversation_start
            ),

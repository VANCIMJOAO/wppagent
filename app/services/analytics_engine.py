...
class AdvancedAnalyticsEngine:
    ...
    async def get_conversion_funnel(
        ...
        try:
            ...
            first_contacts = (
                await self.session.execute(first_contact_query)
            ).scalar() or 0
            ...
            bot_responses = (
                await self.session.execute(bot_responses_query)
            ).scalar() or 0

            # Etapa 3: Usuários que agendaram
            scheduled_query = (
                select(func.count(func.distinct(User.id)))
                .select_from(User.join(Appointment))
            ...
            conversion_rates = {
                "contact_to_response": (
                    (bot_responses / first_contacts * 100) if first_contacts > 0 else 0
                ),
                "response_to_schedule": (
                    (scheduled / bot_responses * 100) if bot_responses > 0 else 0
                ),
                "contact_to_schedule": (
                    (scheduled / first_contacts * 100) if first_contacts > 0 else 0
                ),
                "schedule_to_confirm": (
                    (confirmed / scheduled * 100) if scheduled > 0 else 0
                ),
                "confirm_to_complete": (
                    (completed / confirmed * 100) if confirmed > 0 else 0
                ),
                "overall_conversion": (
                    (completed / first_contacts * 100) if first_contacts > 0 else 0
                ),
            }
            ...
            result = {
                ...
                "funnel_health_score": min(
                    100, conversion_rates["overall_conversion"] * 10
                ),
            ...
            logger.info(
                f"✅ Funil analisado - Conversão geral: {conversion_rates['overall_conversion']:.1f}%"
            )
            return result
        ...
        try:
                ...
                hourly_data.append(
                    {
                        ...
                        "response_rate": (
                            float(row.response_rate) * 100 if row.response_rate else 0
                        ),
                        "efficiency_score": int(row.message_count)
                        / max(1, int(row.unique_users)),
                    }
                )
            ...
            weekly_query = (
                select(
                    ...
                    func.count(
                        func.distinct(
                            case(
                                (Message.direction == "in", Message.user_id), else_=None
                            )
                        )
                ...
                weekly_data.append(
                    {
                        ...
                        "engagement_ratio": int(row.message_count)
                        / max(1, int(row.unique_users)),
                    }
                )
            ...
            daily_query = (
                select(
                    ...
                    func.count(Message.id).label("message_count"),
                    func.count(func.distinct(Message.user_id)).label("unique_users"),
                    func.count(case((Message.direction == "in", 1), else_=None)).label(
                        "incoming_messages"
                    ),
                    func.count(case((Message.direction == "out", 1), else_=None)).label(
                        "outgoing_messages"
                    ),
                )
                .where(Message.created_at.between(start_date, end_date))
                ...
                daily_data.append(
                    {
                        ...
                        "response_ratio": int(row.outgoing_messages)
                        / max(1, int(row.incoming_messages)),
                    }
                )
            ...
            for i, day in enumerate(daily_data):
                ...
                day["messages_ma7"] = sum(d["messages"] for d in window_data) / len(
                    window_data
                )
                day["users_ma7"] = sum(d["unique_users"] for d in window_data) / len(
                    window_data
                )

            # Identificar padrões e insights
            peak_hours = sorted(hourly_data, key=lambda x: x["messages"], reverse=True)[
                :3
            ]
            busiest_days = sorted(
                weekly_data, key=lambda x: x["messages"], reverse=True
            )[:3]

            # Análise de sazonalidade
            weekend_msgs = sum(
                d["messages"] for d in weekly_data if d["day_of_week"] in [0, 6]
            )
            weekday_msgs = sum(
                d["messages"] for d in weekly_data if d["day_of_week"] not in [0, 6]
            )
            ...
            result = {
                ...
                "insights": {
                    ...
                    "activity_score": sum(h["messages"] for h in hourly_data)
                    / len(hourly_data),
            ...
            logger.info(
                f"✅ Análise temporal concluída - {len(daily_data)} dias analisados"
            )
            return result
        ...
        try:
            ...
            vip_query = (
                select(
                    ...
                    func.sum(
                        case(
                            (Appointment.price.is_not(None), Appointment.price), else_=0
                        )
                    ).label("total_spent"),
                    func.max(Appointment.created_at).label("last_appointment"),
                    func.min(Appointment.created_at).label("first_appointment"),
                    func.count(func.distinct(func.date(Appointment.created_at))).label(
                        "visit_days"
                    ),
                )
                .select_from(User.join(Appointment))
                .where(
                    ...
                    >= start_date - timedelta(days=90)  # Expanded window for VIPs
                )
                .group_by(User.id, User.nome, User.telefone)
                .having(func.count(Appointment.id) >= 2)  # At least 2 appointments
            ...
            for row in result:
                days_active = (
                    (row.last_appointment - row.first_appointment).days
                    if row.first_appointment and row.last_appointment
                    else 0
                )
                vip_customers.append(
                    {
                        ...
                        "last_appointment": (
                            row.last_appointment.isoformat()
                            if row.last_appointment
                            else None
                        ),
                        "first_appointment": (
                            row.first_appointment.isoformat()
                        ...
                        "avg_order_value": (
                            float(row.total_spent) / int(row.total_appointments)
                            if row.total_spent and row.total_appointments
                            else 0
                        ),
                        "loyalty_score": min(
                            100,
                            int(row.total_appointments) * 20 + (days_active / 30) * 10,
                        ),
                    }
                )
            ...
            churn_query = (
                ...
                .select_from(User.outerjoin(Message).outerjoin(Appointment))
                .group_by(User.id, User.nome, User.telefone)
                .having(
                    and_(
                    ...
                    churned_customers.append(
                        {
                            ...
                            "lifetime_value": (
                                float(row.lifetime_value) if row.lifetime_value else 0
                            ),
                            "risk_score": min(100, days_inactive * 1.5),
                            "reactivation_priority": (
                                "High"
                                if row.lifetime_value and row.lifetime_value > 200
                                else "Medium"
                            ),
                        }
                    )
            ...
            prospects_query = (
                ...
                .select_from(User.join(Message).outerjoin(Appointment))
                .where(Message.created_at.between(start_date, end_date))
                .group_by(User.id, User.nome, User.telefone)
                .having(
                    and_(
                ...
                high_value_prospects.append(
                    {
                        ...
                        "incoming_messages": int(row.incoming_messages),
                        "last_message": (
                            row.last_message.isoformat() if row.last_message else None
                        ),
                        "engagement_score": engagement_score,
                        "conversion_potential": (
                            "High" if engagement_score > 60 else "Medium"
                        ),
                        "days_since_contact": (
                            (datetime.now() - row.last_message).days
                            if row.last_message
                            else 0
                        ),
                    }
            ...
            avg_customer_value = (
                sum(c["total_spent"] for c in vip_customers) / len(vip_customers)
                if vip_customers
                else 0
            )
            total_ltv = sum(c["total_spent"] for c in vip_customers) + sum(
                c["lifetime_value"] for c in churned_customers
            )
            ...
            result = {
                ...
                "customer_summary": {
                    ...
                    "churn_rate": len(churned_customers)
                    / max(1, total_customers)
                    * 100,
                    "prospect_conversion_opportunity": len(high_value_prospects)
                    * avg_customer_value,
                },
                "segmentation": {
                    "high_value": len(
                        [
                            ...
                            if c["total_spent"] > avg_customer_value
                        ]
                    ),
                    "medium_value": len(
                        [
                            c
                            for c in vip_customers
                            if 0 < c["total_spent"] <= avg_customer_value
                        ]
                    ),
                    "at_risk": len(
                        [c for c in churned_customers if c["days_inactive"] < 90]
                    ),
                    "lost": len(
                        [c for c in churned_customers if c["days_inactive"] >= 90]
                    ),
                ...
                "recommendations": self._generate_customer_recommendations(
                    vip_customers, churned_customers, high_value_prospects
                ),
            }

            logger.info(
                f"✅ Insights de clientes concluídos - {len(vip_customers)} VIPs, {len(high_value_prospects)} prospects"
            )
            return result
    ...
    async def get_business_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            revenue_query = select(
                ...
                func.count(func.distinct(Appointment.user_id)).label(
                    "unique_customers"
                ),
            ).where(
                and_(
            ...
            cac = (
                estimated_marketing_spend / max(1, revenue_data.unique_customers)
                if revenue_data.unique_customers
                else 0
            )

            # Customer Lifetime Value (30-day window)
            ltv_query = (
                select(func.avg(func.sum(Appointment.price)).label("avg_ltv"))
                .select_from(Appointment.join(User))
                .where(
                    and_(
                        Appointment.price.is_not(None),
                        Appointment.created_at
                        >= start_date - timedelta(days=90),  # Expanded window
                    )
                )
                .group_by(User.id)
            )
            ...
            if prev_data and prev_data.prev_revenue:
                current_revenue = (
                    float(revenue_data.total_revenue)
            ...
            result = {
                ...
                "revenue_metrics": {
                    ...
                    "total_appointments": (
                        int(revenue_data.total_appointments)
                        if revenue_data.total_appointments
                        else 0
                    ),
                    "avg_order_value": (
                        float(revenue_data.avg_order_value)
                        if revenue_data.avg_order_value
                        else 0
                    ),
                    "unique_customers": (
                        int(revenue_data.unique_customers)
                        if revenue_data.unique_customers
                        else 0
                    ),
                    "revenue_per_customer": (
                        ...
                        if revenue_data.total_revenue and revenue_data.unique_customers
                        else 0
                    ),
                },
                "customer_metrics": {
                ...
                "efficiency_metrics": {
                    "messages_per_conversion": (
                        total_messages / max(1, revenue_data.unique_customers)
                        if revenue_data.unique_customers
                        else 0
                    ),
                    "cost_per_appointment": (
                        estimated_marketing_spend
                        / max(1, revenue_data.total_appointments)
                        if revenue_data.total_appointments
                        else 0
                    ),
                    "roi_percentage": (
                        ...
                        if estimated_marketing_spend > 0 and revenue_data.total_revenue
                        else 0
                    ),
                },
            }
            ...
            logger.info(
                f"✅ Métricas de negócio calculadas - ROI: {result['efficiency_metrics']['roi_percentage']:.1f}%"
            )
            return result
    ...
    def _generate_funnel_recommendations(self, conversion_rates: Dict) -> List[str]:
        ...
        if conversion_rates["contact_to_schedule"] < 20:
            recommendations.append(
                "📞 Taxa de agendamento baixa - revisar scripts do bot e ofertas"
            )

        if conversion_rates["schedule_to_confirm"] < 70:
            recommendations.append(
                "✅ Muitos agendamentos não confirmados - implementar lembretes automáticos"
            )
        ...
        if conversion_rates["overall_conversion"] > 15:
            recommendations.append(
                "🚀 Performance excelente - considerar escalar operação"
            )

        return recommendations

    def _generate_time_recommendations(
        self, hourly_data: List, weekly_data: List
    ) -> List[str]:
        ...
        if weekend_activity > weekday_activity * 0.3:
            recommendations.append(
                "📅 Boa atividade nos finais de semana - manter atendimento"
            )

        return recommendations

    def _generate_customer_recommendations(
        self, vips: List, churned: List, prospects: List
    ) -> List[str]:
        ...
        if len(vips) > 0:
            recommendations.append(
                f"👑 {len(vips)} clientes VIP identificados - criar programa de fidelidade"
            )

        if len(churned) > 0:
            recommendations.append(
                f"📧 {len(churned)} clientes inativos - campanha de reativação"
            )

        if len(prospects) > 0:
            recommendations.append(
                f"🎯 {len(prospects)} prospects engajados - ofertas personalizadas"
            )

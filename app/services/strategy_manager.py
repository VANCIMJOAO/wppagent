...
class StrategyManager:
    ...
    def __init__(self):
        ...
        logger.info(
            f"StrategyManager inicializado com {len(self.strategies)} estratégias"
        )
    ...
    async def process(self, message: str, user_id: str, phone: str) -> StrategyResponse:
        ...
        try:
            ...
            self.metrics_service.track_message(
                direction="in", message_type="text", status="processing"
            )
            ...
            with self.metrics_service.track_component_performance("context_building"):
                context = await self._build_context_with_state(
                    message, user_id, phone, conversation_state
                )

            # Seleciona estratégia apropriada
            with self.metrics_service.track_component_performance("strategy_selection"):
                selected = self.selector.select_strategy(
            ...
            if not selected:
                ...
                self.metrics_service.track_message(
                    direction="out", message_type="text", status="error"
                )
                self.metrics_service.track_error(
                    error_type="no_strategy_available", component="strategy_manager"
                )
            ...
            self.metrics_service.track_strategy_selection(
                strategy=strategy_name, reason="context_based_selection", success=True
            )
            ...
            if result.success and result.response:
                ...
                if any(
                    word in message_lower for word in ["agendar", "marcar", "horário"]
                ) or any(
                    word in response_lower
                    for word in ["agendar", "agendamento", "horário"]
                ):
                    ...
                    if "confirmar" in response_lower:
                        await state_manager.update_booking_context(
                            user_id, step="confirmation"
                        )
                    elif "serviço" in response_lower:
                        await state_manager.update_booking_context(
                            user_id, step="service_selection"
                        )
                    elif "horário" in response_lower or "data" in response_lower:
                        await state_manager.update_booking_context(
                            user_id, step="datetime"
                        )
    ...
    async def _build_context(
        self, message: str, user_id: str, phone: str
    ) -> MessageContext:
        """
        ...
        try:
            ...
            cached_lead_score = await cache_service.get_cached_lead_score(
                user_id, message
            )
            ...
            if cached_lead_score:
                ...
                try:
                    ...
                    if hasattr(lead_score_result, "total_score"):
                        ...
                        confidence = float(
                            getattr(lead_score_result, "confidence", 0.5)
                        )
                        ...
                        await cache_service.cache_lead_score(
                            ...
                            lead_score_data={
                                ...
                                "category": (
                                    getattr(
                                        lead_score_result, "category", "unknown"
                                    ).value
                                    if hasattr(
                                        getattr(lead_score_result, "category", None),
                                        "value",
                                    )
                                    else str(
                                        getattr(
                        ...
                        self.metrics_service.track_lead_score(
                            score=lead_score,
                            category=str(
                                getattr(lead_score_result, "category", "unknown")
                            ),
                        ...
                        logger.warning(
                            f"Lead scoring retornou formato inválido para {user_id}"
                        )
    ...
    async def _build_context_with_state(
        self, message: str, user_id: str, phone: str, conversation_state
    ) -> MessageContext:
        """
    ...
    def get_stats(self) -> dict:
        ...
        average_response_time = (
            ...
            if self.metrics.get("response_times")
            else 0
        )

...
class WhatsAppAgentCrew:
    ...
    def _ensure_crewai_loaded(self):
        ...
        if not self._crewai_initialized:
            if not _load_crewai():
                raise RuntimeError(
                    "CrewAI não pôde ser carregado. Verifique as permissões."
                )
    ...
    def process_message(
        ...
        try:
            ...
            intent = self._analyze_intent_with_flow(
                message, context, lead_score, flow_decision
            )

            # 4. Seleção de agente baseada em fluxo e score
            selected_agent = self._select_agent_with_flow(
                intent, lead_score, flow_decision
            )
    ...
    def _analyze_intent_with_flow(
        ...
        if lead_score and priority_level >= 4:
            ...
            if any(
                ...
                for word in ["urgente", "agora", "hoje", "imediato"]
            ):
                return "escalation"

            # Leads qualificados falando de serviços = vendas diretas
            if any(
                word in message_lower for word in ["quero", "preciso", "vou contratar"]
            ):
        ...
        if any(
            word in message_lower
            for word in ["agendar", "marcar", "horário", "data", "disponibilidade"]
        ):
            return "agendamento"

        if any(
            word in message_lower
            for word in ["preço", "valor", "quanto custa", "orçamento"]
        ):
            return "consulta_preco"

        if any(
            word in message_lower
            for word in ["cancelar", "desmarcar", "problema", "reclamação"]
        ):
            return "suporte"

        if any(
            word in message_lower
            for word in ["onde", "endereço", "localização", "como chegar"]
        ):
            return "informacao"

        if any(
            word in message_lower
            for word in ["corte", "barba", "sobrancelha", "massagem", "serviço"]
        ):
            return "consulta_servico"

        if any(
            word in message_lower
            for word in ["olá", "oi", "bom dia", "boa tarde", "boa noite"]
        ):
            return "saudacao"

        return "geral"

    def _select_agent_with_flow(
        ...
        if lead_score:
            ...
            if priority_level >= 4 and intent in [
                "vendas_qualificada",
                "consulta_preco",
            ]:
                return AgentRole.SALES
    ...
    def _create_task_with_flow(
        ...
        if lead_score:
            ...
            category = getattr(lead_score, "category", None)
            category_value = (
                getattr(category, "value", "INDEFINIDA") if category else "INDEFINIDA"
            )
            priority_level = getattr(lead_score, "priority_level", 2)
            estimated_value = getattr(lead_score, "estimated_value", 0.0)
    ...
    async def process_complex_request(
        ...
        try:
            ...
            result = self.process_message(
                message=user_message, user_phone=user_id, context=context_data
            )
...
class WhatsAppCrewManager:
    ...
    async def process_complex_request(
        ...
        return await crew.process_complex_request(
            user_message, agent_role, user_id, context_data
        )
    ...
    def get_crew_analytics(self) -> Dict[str, Any]:
        ...
        if crew is None:
            return {
                "status": "unavailable",
                "error": (
                    str(self._crew_error)
                    if self._crew_error
                    else "CrewAI não inicializado"
                ),
                "crewai_loaded": False,
                "performance": {},
                "agents": {},

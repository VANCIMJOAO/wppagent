    ...
    async def execute(self, context: MessageContext) -> StrategyResponse:
        ...
        try:
            ...
            return StrategyResponse(
                ...
                metadata={**llm_response.metadata, "processing_method": "advanced_llm"},
            )
...
class CrewAIStrategy(BaseStrategy):
    ...
    def _select_agent(self, context: MessageContext) -> AgentRole:
        ...
        if any(word in message_lower for word in ["agendar", "marcar", "horário"]):
            ...
            word in message_lower for word in ["preço", "valor", "comprar", "contratar"]
        ):
            return AgentRole.SALES_SPECIALIST
        elif any(
            word in message_lower for word in ["problema", "reclamação", "cancelar"]
        ):
...
class HybridStrategy(BaseStrategy):
    ...
    async def execute(self, context: MessageContext) -> StrategyResponse:
        ...
        try:
            ...
            needs_crew_refinement = self._needs_crew_refinement(context, llm_response)
            ...
            if needs_crew_refinement:
                ...
                return StrategyResponse(
                    ...
                    interactive_buttons=crew_response.interactive_buttons
                    or llm_response.interactive_buttons,
                    suggested_actions=(crew_response.suggested_actions or [])
                    + (llm_response.suggested_actions or []),
                    metadata={
    ...
    def _needs_crew_refinement(
        self, context: MessageContext, llm_response: StrategyResponse
    ) -> bool:

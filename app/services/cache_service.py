...
class CacheService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"CacheService inicializado - Redis: {self.redis_available}, Enabled: {self.enabled}"
        )
        ...
        try:
            if self.redis:
                ...
                if key in self.memory_cache:
                    ...
                    if (
                        timestamp
                        and (datetime.now() - timestamp).total_seconds() < 3600
                    ):  # TTL padrão
    ...
    def _cleanup_memory_cache(self):
        ...
        try:
            ...
            if expired_keys:
                logger.debug(
                    f"Limpeza do cache em memória: {len(expired_keys)} chaves expiradas removidas"
                )
    ...
    # CACHE DE LEAD SCORE
    async def get_cached_lead_score(
    ...
    async def invalidate_user_cache(self, user_id: str):
        ...
        try:
            if self.redis:
                ...
                for pattern in patterns:
                    try:
                        ...
                        if keys:
                            ...
                            logger.debug(
                                f"Invalidado cache do usuário {user_id}: {len(keys)} chaves"
                            )
                    except:
                        pass
            else:
                ...
                if keys_to_remove:
                    logger.debug(
                        f"Invalidado cache em memória do usuário {user_id}: {len(keys_to_remove)} chaves"
                    )
        ...
        try:
            ...
            if self.redis:
                ...
                if keys:
                    self.redis.delete(*keys)
                    logger.info(
                        f"Invalidado cache do tipo {cache_type.value}: {len(keys)} chaves"
                    )
            else:
                # Cache em memória
                keys_to_remove = [
                ...
                if keys_to_remove:
                    logger.info(
                        f"Invalidado cache em memória do tipo {cache_type.value}: {len(keys_to_remove)} chaves"
                    )
        ...
        try:
            if self.redis:
                ...
                if keys:
                    self.redis.delete(*keys)
                    logger.info(
                        f"Cache Redis totalmente limpo: {len(keys)} chaves removidas"
                    )
            else:
                # Cache em memória
                ...
                logger.info(
                    f"Cache em memória totalmente limpo: {count} chaves removidas"
                )
    ...
    async def get_or_set(
        self, key: str, fetch_function, ttl: int = 3600, cache_type: str = "general"
    ):
        """
        ...
        try:
            ...
            if hasattr(result, "__iter__") and not isinstance(result, str):
                ...
                await self._set_to_cache(
                    key, json.dumps(serialized_result, default=str), ttl
                )
            elif hasattr(result, "model_dump"):
                # Handle single Pydantic model
                await self._set_to_cache(
                    key, json.dumps(result.model_dump(), default=str), ttl
                )
            else:

...
from ..models.database import Base, User, Conversation, Appointment, Message
...
class LGPDComplianceManager:
    ...
    def _initialize_data_maps(self) -> Dict[str, LGPDDataMap]:
        ...
        return {
            ...
            "conversations": LGPDDataMap(
                ...
                columns=["id", "user_id", "phone_number", "context", "created_at", "updated_at"],
            ...
            "messages": LGPDDataMap(
                category=LGPDDataCategory.CONVERSATION,
                table_name="messages",
                columns=["id", "conversation_id", "content", "sender_type", "created_at"],
                retention_period=LGPDRetentionPeriod.YEARS_2, 
                processing_purpose=LGPDProcessingPurpose.SERVICE_EXECUTION,
                legal_basis="Art. 7º, V - execução de contrato"
            ),
            "appointments": LGPDDataMap(
                ...
                columns=["id", "user_id", "client_name", "client_phone", "service_type", "scheduled_at", "notes"],
                retention_period=LGPDRetentionPeriod.YEARS_5,
    ...
    async def get_user_data_for_portability(self, user_identifier: str) -> LGPDDataPortability:
        """
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                # Coletar conversas
                conversations = await self._get_user_conversations(session, user_id, phone_number)
                
                # Coletar agendamentos  
                appointments = await self._get_user_appointments(session, user_id, phone_number)
                ...
                logger.info(f"✅ Dados coletados: {len(conversations)} conversas, {len(appointments)} agendamentos")
    ...
    async def _get_user_basic_data(self, session: Session, user_identifier: str) -> Optional[Dict[str, Any]]:
    ...
    async def _get_user_conversations(self, session: Session, user_id: str, phone_number: str) -> List[Dict[str, Any]]:
    ...
    async def _get_user_appointments(self, session: Session, user_id: str, phone_number: str) -> List[Dict[str, Any]]:
    ...
    async def _get_user_client_data(self, session: Session, phone_number: str) -> Optional[Dict[str, Any]]:
    ...
    async def export_user_data(self, user_identifier: str) -> str:
        ...
        try:
            ...
            export_id = hashlib.md5(f"{user_identifier}_{timestamp}".encode()).hexdigest()[:8]
            ...
            if portability_data.conversations:
                ...
                with open(conversations_file, 'w', encoding='utf-8') as f:
                    json.dump(portability_data.conversations, f, ensure_ascii=False, indent=2)
            ...
            if portability_data.appointments:
                ...
                with open(appointments_file, 'w', encoding='utf-8') as f:
                    json.dump(portability_data.appointments, f, ensure_ascii=False, indent=2)
    ...
    async def delete_user_account(self, user_identifier: str, reason: str = "user_request") -> Dict[str, Any]:
        """
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                conversations_deleted = await self._delete_user_conversations(session, user_id, phone_number)
                deletion_summary["deleted_records"]["conversations"] = conversations_deleted
                
                # 3. Anonimizar agendamentos (manter por obrigação legal)
                appointments_anonymized = await self._anonymize_user_appointments(session, user_id, phone_number)
                deletion_summary["anonymized_records"]["appointments"] = appointments_anonymized
    ...
    async def _delete_user_conversations(self, session: Session, user_id: str, phone_number: str) -> int:
        ...
        try:
            ...
            if conversation_ids:
                # Deletar mensagens
                msg_stmt = delete(Message).where(Message.conversation_id.in_(conversation_ids))
                await session.execute(msg_stmt)
                
                # Deletar conversas
                conv_delete_stmt = delete(Conversation).where(Conversation.id.in_(conversation_ids))
    ...
    async def _anonymize_user_appointments(self, session: Session, user_id: str, phone_number: str) -> int:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                for data_key, data_map in self.data_maps.items():
                    ...
                    records_processed = await self._apply_retention_policy(session, data_map)
                    retention_summary["policies_applied"][data_key] = records_processed
                    retention_summary["total_records_processed"] += records_processed.get("processed", 0)
                    retention_summary["total_records_deleted"] += records_processed.get("deleted", 0)
                    retention_summary["total_records_anonymized"] += records_processed.get("anonymized", 0)
    ...
    async def _apply_retention_policy(self, session: Session, data_map: LGPDDataMap) -> Dict[str, int]:
        """Aplica política de retenção para um mapeamento específico"""
        try:
            cutoff_date = datetime.utcnow() - timedelta(days=data_map.retention_period.value)
            ...
            if data_map.processing_purpose == LGPDProcessingPurpose.LEGAL_COMPLIANCE:
                ...
                await session.execute(anonymize_query, {
                    "cutoff_date": cutoff_date,
                    "now": datetime.utcnow()
                })
                
                return {"processed": expired_count, "deleted": 0, "anonymized": expired_count}
            else:
                ...
                return {"processed": expired_count, "deleted": expired_count, "anonymized": 0}
                
        except Exception as e:
            logger.error(f"Erro ao aplicar política de retenção para {data_map.table_name}: {e}")
            return {"processed": 0, "deleted": 0, "anonymized": 0}
    ...
    async def get_data_processing_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Relatório gerado: {report['total_records']} registros analisados")
            return report

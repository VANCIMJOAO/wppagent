...
class IntelligentHandoffService:
    ...
    def _load_handoff_config(self, config_path: str = None) -> Dict:
        ...
        if config_path:
            try:
                ...
                logger.warning(
                    f"Erro ao carregar config {config_path}: {e}. Usando config padrão."
                )

        return default_config

    async def analyze_message_for_handoff(
        self, user_id: str, message: str, conversation_history: List[Dict] = None
        ...
        try:
            ...
            for rule in self.config["handoff_rules"]:
                for pattern in rule["trigger_patterns"]:
                    if re.search(pattern, message_lower):
                        logger.info(
                            f"🎯 Handoff detectado: {rule['name']} para usuário {user_id}"
                        )

                        # Determinar nível de escalação
                        escalation_level = rule["escalation_level"]
                        escalation_config = self.config["escalation_paths"][
                            escalation_level
                        ]
                        ...
                        is_business_hour = testing_mode or self._is_business_hours(
                            escalation_config["available_hours"]
                        )
            ...
            behavioral_handoff = await self._check_behavioral_triggers(
    ...
    async def _handle_outside_hours(
        self, user_id: str
    ) -> Tuple[bool, HandoffReason, Dict]:
    ...
    async def should_prevent_handoff(
        ...
        if metrics["escalation_count"] >= 3:
            return (
                True,
                "Vamos tentar resolver isso juntos antes de buscar mais ajuda. Pode me dar mais detalhes sobre o que precisa?",
            )
    ...
    def get_handoff_summary(self, user_id: str) -> Dict:
        ...
        return self.conversation_metrics.get(
            user_id, {"handoffs": [], "escalation_count": 0, "last_handoff": None}
        )

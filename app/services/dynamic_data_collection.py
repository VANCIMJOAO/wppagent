...
class DynamicDataCollectionService:
    ...
    async def get_or_create_data_collection(
        self, user_id: int
        ...
        if not data_collection:
            data_collection = CustomerDataCollection(
                user_id=user_id, collection_method="whatsapp"
            )
    ...
    async def request_missing_data(self, user_id: int, missing_field: str) -> str:
        ...
        message = template or fallback_messages.get(
            missing_field, "Por favor, forneça a informação solicitada:"
        )
    ...
    async def validate_and_save_data(
        ...
        if field == "name":
            ...
            if not re.match(email_regex, value):
                return (
                    False,
                    "Por favor, forneça um email válido (exemplo: nome@email.com)",
                )
    ...
    async def get_collection_progress(self, user_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_retries": (
                (await self.get_bot_config()).max_retries_data_collection
                if await self.get_bot_config()
                else 3
            ),
        }
    ...
    async def get_welcome_message(self, customer_name: str = None) -> str:
        ...
        if template and customer_name:
            ...
            return template.replace("{customer_name}", customer_name).replace(

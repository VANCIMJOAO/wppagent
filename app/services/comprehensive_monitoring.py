...
class SLAMonitor:
    ...
    async def record_metric(
        self, metric: SLAMetric, value: float, labels: Dict[str, str] = None
    ):
        """Registrar uma métrica de SLA"""
        timestamp = datetime.now(timezone.utc)
        metric_data = MetricData(
            name=metric.value, value=value, timestamp=timestamp, labels=labels
        )
        ...
        logger.debug(
            f"SLA metric recorded: {metric.value}={value}",
            {"metric": metric.value, "value": value, "labels": labels},
        )

    async def _check_sla_threshold(
        self, metric: SLAMetric, value: float, timestamp: datetime
    ):
        ...
        if severity:
            await self._create_sla_alert(
                metric, value, threshold_value, severity, timestamp
            )
    ...
    def get_sla_status(self, period_minutes: int = 60) -> Dict[str, Any]:
        ...
        for metric, threshold in self.thresholds.items():
            recent_metrics = [
                m for m in self.metrics_buffer[metric] if m.timestamp >= since
            ]
            ...
            if recent_metrics:
                ...
                if metric == SLAMetric.RESPONSE_TIME:
                    ...
                    current_value = self.uptime_tracker.get_uptime_percentage(
                        period_minutes
                    )
                    is_healthy = current_value >= threshold.warning_threshold
                else:
        ...
        return {
            ...
            "metrics": status,
            "overall_health": all(s.get("is_healthy", True) for s in status.values()),
...
class UptimeTracker:
    ...
    def record_uptime(self):
        ...
        self.uptime_events.append({"type": "heartbeat", "timestamp": now})
    ...
    def record_downtime(self):
        ...
        if not self.downtime_start:
            ...
            self.uptime_events.append({"type": "downtime", "timestamp": now})
    ...
    def get_uptime_percentage(self, period_minutes: int = 60) -> float:
        ...
        recent_events = [e for e in self.uptime_events if e["timestamp"] >= since]
        ...
        for event in recent_events:
            if event["type"] == "downtime":
                ...
                downtime_seconds += (
                    event["timestamp"] - current_downtime_start
                ).total_seconds()
                current_downtime_start = None
...
class BusinessMetricsCollector:
    ...
    async def record_business_metric(
        self, metric: BusinessMetric, value: float, labels: Dict[str, str] = None
    ):
        """Registrar métrica de negócio"""
        timestamp = datetime.now(timezone.utc)
        metric_data = MetricData(
            name=metric.value, value=value, timestamp=timestamp, labels=labels or {}
        )
        ...
        logger.debug(
            f"Business metric recorded: {metric.value}={value}",
            {"metric": metric.value, "value": value, "labels": labels},
        )
    ...
    def get_business_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        for metric in BusinessMetric:
            metric_summary = {"current_period": {}, "trends": [], "totals": {}}
            ...
            if len(daily_values) >= 2:
                recent_avg = (
                    statistics.mean(daily_values[-3:])
...
class MonitoringSystem:
    ...
    async def _health_check_loop(self):
        ...
        while self.running:
            try:
                ...
                await self.sla_monitor.record_metric(
                    SLAMetric.RESPONSE_TIME, response_time
                )
    ...
    async def _metrics_cleanup_loop(self):
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"Limpeza de métricas antigas executada (>{retention_days} dias)"
                )
    ...
    async def record_api_call(
        ...
        if endpoint.startswith("/webhook"):
            await self.business_metrics.record_business_metric(
                BusinessMetric.MESSAGES_PROCESSED,
                1.0,
                {"status_code": str(status_code), "user_id": user_id or "unknown"},
            )

    async def record_business_event(
        self, event_type: str, value: float = 1.0, metadata: Dict[str, Any] = None
    ):
    ...
    def get_monitoring_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_alerts": [
                alert.to_dict() for alert in self.sla_monitor.active_alerts.values()
            ],
            "system_info": {
                "monitoring_enabled": self.config.metrics_enabled,
                "alerting_enabled": self.config.alerting_enabled,
                "uptime_percentage": self.sla_monitor.uptime_tracker.get_uptime_percentage(
                    1440
                ),  # 24h
                "config": {
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.config.alert_slack_webhook, json=payload
                ) as response:
...
async def record_business_event(
    event_type: str, value: float = 1.0, metadata: Dict[str, Any] = None
):
    """Registrar evento de negócio"""

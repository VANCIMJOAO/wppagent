...
class ServiceData:
    ...
    def __init__(
        self, id: int, name: str, price: str, duration: int, description: str = ""
    ):
        self.id = id
        self.name = name
...
class BusinessDataService:
    ...
    async def get_active_services(
        ...
        if self._services_cache is None or refresh_cache:
            ...
            try:
                ...
                logger.info(
                    f"✅ Carregados {len(self._services_cache)} serviços da database"
                )
    ...
    async def get_services_formatted_text(self, user_message: str = "") -> str:
        ...
        if (
            "mais serviços" in user_message.lower()
            or "restante" in user_message.lower()
        ):
    ...
    async def find_service_by_name(self, service_name: str) -> Optional[ServiceData]:
        ...
        for service in services:
            ...
            if any(word in service.name.lower() for word in search_words):
                logger.info(
                    f"✅ Encontrado por palavra-chave: {service.name} (busca: {search_words})"
                )
                return service
        ...
        for service in services:
            ...
            if any(keyword in service_name_clean for keyword in all_keywords):
                logger.info(
                    f"✅ Encontrado por sinônimo: {service.name} (termo: '{service_name_clean}')"
                )
                return service
            ...
            search_words = service_name_clean.split()
            if any(
                search_word in keyword
                for keyword in all_keywords
                for search_word in search_words
            ):
                logger.info(
                    f"✅ Encontrado por keyword reversa: {service.name} (busca: {search_words})"
                )
                return service
        ...
        for service in services:
            service_words = set(
                service.name.lower().replace("-", " ").replace("_", " ").split()
            )
            search_words = set(
                service_name_clean.replace("-", " ").replace("_", " ").split()
            )
            ...
            if common_words:
                logger.info(
                    f"✅ Encontrado por interseção: {service.name} (palavras comuns: {common_words})"
                )
                return service

        logger.warning(
            f"❌ Serviço não encontrado: '{service_name}' (testados {len(services)} serviços)"
        )
        return None

    async def find_services_by_keyword(
        ...
        if self._payment_methods_cache is None or refresh_cache:
            ...
            try:
                ...
                if payment_methods:
                    ...
                    logger.info(
                        f"✅ Carregadas {len(self._payment_methods_cache)} formas de pagamento"
                    )
                else:
                    # Fallback padrão
                    self._payment_methods_cache = [
    ...
    def _get_default_payment_methods(self) -> List[Dict]:
        ...
        return [
            ...
            },
            {
                ...
                "additional_info": "Chave PIX disponível",
            },
            {
                "name": "Cartão de Débito",
            ...
            },
            {
                ...
                "additional_info": "Parcelamento disponível",
            },
        ]
    ...
    async def get_services_dict(self) -> Dict[str, int]:
        ...
        return {service.name.lower(): service.duration for service in services}
    ...
    async def get_service_price_accurate(
        self, service_query: str
        ...
        try:
            ...
            if query_lower in self.service_synonyms:
                ...
                if isinstance(synonym_target, list):
                    ...
                    found_services = [
                        s
                        for s in services
                        if s["name"].lower() in [t.lower() for t in synonym_target]
                    ]
                    if found_services:
                        return {
                            **found_services[0],
                    ...
                    found_service = next(
                        (
                            s
                            for s in services
                            if s["name"].lower() == synonym_target.lower()
                        ),
                        None,
                    )
                    if found_service:
            ...
            for category, service_names in self.search_terms.items():
                if category in query_lower:
                    ...
                    category_services = [
                        s
                        for s in services
                        if s["name"].lower() in [name.lower() for name in service_names]
                    ]
            ...
            for service in services:
                similarity = self._calculate_text_similarity(
                    query_lower, service["name"].lower()
                )
                if (
    ...
    async def get_services_by_names(
    ...
    def _service_belongs_to_category(self, service_name: str, category: str) -> bool:
        ...
        if category in self.search_terms:
            return service_name.lower() in [
                name.lower() for name in self.search_terms[category]
            ]
        return False
    ...
    async def _analyze_user_context(self, user_query: str) -> Dict[str, Any]:
        ...
        for synonym, target in self.service_synonyms.items():
            if synonym in query_lower:
                context["specific_service"] = (
                    target if isinstance(target, str) else target[0]
                )
                context["confidence"] += 0.3
                break
    ...
    async def format_services_by_category(
        self, category: str, include_prices: bool = True
    ) -> str:
    ...
    async def format_services_paginated(self, page: int = 1, per_page: int = 8) -> str:
        ...
        try:
            ...
            formatted = (
                f"📋 *Nossos Serviços e Preços (Parte {page}/{total_pages}):*\n\n"
            )

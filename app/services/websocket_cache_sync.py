...
class WebSocketCacheSync:
    ...
    async def connect(
        ...
        try:
            ...
            logger.info(
                f"🔌 WebSocket conectado: {connection_id} (Total: {len(self.connections)})"
            )
            return True
    ...
    async def disconnect(self, connection_id: str):
        ...
        try:
            ...
            logger.info(
                f"🔌 WebSocket desconectado: {connection_id} (Total: {len(self.connections)})"
            )
    ...
    async def broadcast_cache_invalidation(
        ...
        try:
            ...
            logger.debug(
                f"📢 Cache invalidation broadcast: {event.value} "
    ...
    async def start_heartbeat(self):
        ...
        while True:
            try:
                ...
                logger.debug(
                    f"💗 Heartbeat enviado: ✅{result['successful_sends']} "

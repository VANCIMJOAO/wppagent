...
class RetryHandler:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if circuit_breaker.state == CircuitState.HALF_OPEN:
                    ...
                    and circuit_breaker.failure_count > 0
                ):
                ...
                last_exception = e
                logger.warning(
                    f"Tentativa {attempt + 1} falhou para {service_name}: {str(e)}"
                )
                ...
                if attempt < max_retries:
                    ...
                    logger.info(
                        f"Aguardando {total_delay:.2f}s antes da próxima tentativa para {service_name}"
                    )
                    await asyncio.sleep(total_delay)

        # Todas as tentativas falharam
        logger.error(
            f"Todas as {max_retries + 1} tentativas falharam para {service_name}"
        )
    ...
    def _handle_failure(
        self, circuit_breaker: CircuitBreaker, service_name: str, exception: Exception
    ):
        ...
        if (
            ...
            and circuit_breaker.state == CircuitState.CLOSED
        ):
            circuit_breaker.state = CircuitState.OPEN
            logger.error(
                f"Circuit breaker ABERTO para {service_name} após {circuit_breaker.failure_count} falhas"
            )
    ...
    def _reset_circuit_breaker(
        self, circuit_breaker: CircuitBreaker, service_name: str
    ):

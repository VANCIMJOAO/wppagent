...
from .lead_scoring import LeadCategory, lead_scoring_service
from .llm_advanced import ConversationState, LLMResponse, advanced_llm_service
...
class HybridLLMCrewService:
    ...
    async def process_message(
        ...
        try:
            ...
            logger.info(
                f"Estratégia selecionada: {processing_strategy} para: {message[:50]}..."
            )
            ...
            self._update_performance_metrics(
                processing_strategy, result.get("success", True), response_time
            )
    ...
    async def _quick_llm_analysis(
        self, user_id: str, conversation_id: str, message: str, message_type: str
    ) -> Dict[str, Any]:
    ...
    def _decide_strategy(
        ...
        try:
            ...
            if not lead_score:
                logger.warning(
                    "Lead score não retornou resultado válido, usando estratégia LLM only"
                )
                return "llm_only"
            ...
            try:
                ...
                priority_level = (
                    int(priority_level) if priority_level is not None else 2
                )
            except (ValueError, TypeError) as e:
                logger.warning(
                    f"Erro na conversão de tipos auxiliares do lead score: {e}, usando valores padrão"
                )
                confidence = 0.5
                priority_level = 2

            logger.info(
                f"Lead Score processado: score={total_score}, confidence={confidence}, category={category}, priority={priority_level}"
            )
            ...
            if word_count <= 3 and total_score < 40:
                logger.info(
                    f"Estratégia llm_only por simplicidade: words={word_count}, score={total_score}"
                )
                return "llm_only"
            ...
            if any(keyword in message_lower for keyword in complex_keywords):
                ...
                if total_score >= 50:
                    logger.info(
                        f"Estratégia crew_only por complexidade e score: {total_score}"
                    )
                    return "crew_only"
                else:
                    logger.info(
                        f"Estratégia hybrid por complexidade e score baixo: {total_score}"
                    )
                    return "hybrid"
    ...
    async def _process_hybrid(
        ...
        try:
            ...
            crew_task = self._process_with_crew_only(user_id, message, analysis)
    ...
    def _calculate_response_quality(self, result: Dict[str, Any], source: str) -> float:
        ...
        if (
            ...
            and len(result.get("suggested_actions", [])) > 0
        ):
    ...
    def _update_performance_metrics(
        self, strategy: str, success: bool, response_time: float
    ):
    ...
    def _generate_optimization_recommendations(self) -> List[str]:
        ...
        if total_requests < 100:
            recommendations.append("Colete mais dados para análises mais precisas")

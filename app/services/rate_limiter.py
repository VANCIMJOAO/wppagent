...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.specific_configs: Dict[str, RateLimitConfig] = {
            "webhook": RateLimitConfig(
                max_requests=120, time_window=60, burst_limit=20
            ),
            "api": RateLimitConfig(
                max_requests=30, time_window=60, burst_limit=8
            ),  # Equilibrado para API geral
            "health": RateLimitConfig(
                max_requests=200, time_window=60, burst_limit=100
            ),  # Permissivo mas com limite
            "default": RateLimitConfig(
                max_requests=50, time_window=60, burst_limit=15
            ),  # Equilibrado para outros endpoints
        }
    ...
    def _clean_old_requests(self, client: ClientInfo, time_window: int):
        ...
        client.requests = [
            req_time for req_time in client.requests if req_time > cutoff_time
        ]
    ...
    def _block_client(self, client: ClientInfo, duration_seconds: int = 300):
        ...
        logger.warning(
            f"Cliente bloqueado por {duration_seconds}s (total bloqueios: {client.blocked_count})"
        )

    async def is_allowed(
        self, client_id: str, endpoint: str = "default", cost: int = 1
    ) -> tuple[bool, dict]:
        """
        ...
        if self._is_blocked(client):
            return False, {
                ...
                "retry_after": int(
                    (client.blocked_until - datetime.now()).total_seconds()
                ),
            }
        ...
        recent_requests = len(
            [
                req_time
                for req_time in client.requests
    ...
    def get_client_stats(self, client_id: str) -> dict:
        ...
        return {
            ...
            "blocked_until": (
                client.blocked_until.isoformat() if client.blocked_until else None
            ),
    ...
    def get_global_stats(self) -> dict:
        ...
        blocked_clients = sum(
            1 for client in self.clients.values() if self._is_blocked(client)
        )
    ...
    async def cleanup_old_clients(self, max_age_hours: int = 24):
        ...
        for client_id, client in self.clients.items():
            ...
            if (
                not client.requests or max(client.requests) < cutoff_time
            ) and not self._is_blocked(client):
        ...
        if clients_to_remove:
            logger.info(
                f"Removidos {len(clients_to_remove)} clientes antigos do rate limiter"
            )
...
class WhatsAppRateLimiter(RateLimiter):
    ...
    def __init__(self):
        ...
        self.specific_configs = {
            "incoming_message": RateLimitConfig(
                max_requests=20, time_window=60, burst_limit=5
            ),
            "outgoing_message": RateLimitConfig(
                max_requests=15, time_window=60, burst_limit=3
            ),
            "webhook": RateLimitConfig(
                max_requests=100, time_window=60, burst_limit=10
            ),
        }
        ...
        self.message_type_limits = {
            ...
            "text": {
                ...
                "time_window": 60,
                "burst_limit": 8,
            },  # 30 textos por minuto
            "document": {
                "max_requests": 3,
                "time_window": 60,
                "burst_limit": 1,
            },  # 3 documentos por minuto
            "video": {
                "max_requests": 2,
                "time_window": 60,
                "burst_limit": 1,
            ...
            "sticker": {
                ...
                "time_window": 60,
                "burst_limit": 5,
            },  # 15 stickers por minuto
            "location": {
                "max_requests": 8,
                "time_window": 60,
                "burst_limit": 2,
            },  # 8 localizações por minuto
            "contact": {
                "max_requests": 5,
                "time_window": 60,
                "burst_limit": 2,
            },  # 5 contatos por minuto
            "interactive": {
                "max_requests": 20,
                "time_window": 60,
                "burst_limit": 5,
            },  # 20 interações por minuto
        }

    async def check_message_type_limit(
        self, user_id: str, message_type: str
    ) -> tuple[bool, dict]:
        """
        ...
        type_limits = self.message_type_limits.get(
            message_type, self.message_type_limits["text"]
        )
        ...
        if recent_requests >= type_limits["burst_limit"]:
            self._block_client(
                client, duration_seconds=30
            )  # Bloquear por 30 segundos para tipo específico
            return False, {
        ...
        if len(client.requests) >= type_limits["max_requests"]:
            ...
            if len(client.requests) > type_limits["max_requests"] * 2:
                self._block_client(
                    client, duration_seconds=180
                )  # 3 minutos para abuso de tipo
                return False, {
            ...
            oldest_request = min(client.requests)
            retry_after = int(
                type_limits["time_window"] - (current_time - oldest_request)
            )
    ...
    async def check_combined_message_limits(
        self, user_id: str, message_type: str
    ) -> tuple[bool, dict]:
        """
        ...
        type_allowed, type_info = await self.check_message_type_limit(
        ...
        if message_type:
            ...
            if client_key in self.clients:
                ...
                stats[message_type] = {
                    ...
                    "is_blocked": self._is_blocked(client),
                    "blocked_until": (
                        client.blocked_until.isoformat()
    ...
    async def check_user_message_limit(self, wa_id: str) -> tuple[bool, dict]:
        ...
        return await self.is_allowed(
            client_id=f"user_{wa_id}", endpoint="incoming_message", cost=1
        )

    async def check_outgoing_limit(self, wa_id: str) -> tuple[bool, dict]:
        """Verifica limite para mensagens enviadas"""
        return await self.is_allowed(
            client_id=f"outgoing_{wa_id}", endpoint="outgoing_message", cost=1
        )

...
class LeadScoringEngine:
    ...
    def calculate_lead_score(
        ...
        recommendations = self._generate_recommendations(
            total_score, factors, customer_profile
        )
    ...
    def _analyze_message_behavior(self, message: str) -> Dict[str, float]:
        ...
        urgency_count = sum(
            1 for keyword in self.urgency_keywords if keyword in message_lower
        )
        factors["urgency_indicators"] = min(10, urgency_count * 3)

        # Sensibilidade a preço
        price_count = sum(
            1 for keyword in self.price_keywords if keyword in message_lower
        )
        ...
        if any(
            ...
            for word in ["corte", "barba", "sobrancelha", "massagem"]
        ):
        ...
        authority_count = sum(
            1 for phrase in self.decision_authority if phrase in message_lower
        )
    ...
    def _analyze_demographics(
        self, profile: CustomerProfile, context: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        if profile.geographic_location:
            ...
            if any(
                area in profile.geographic_location.lower() for area in high_value_areas
            ):
                geographic_score = 6
            else:
                geographic_score = 3
    ...
    def _analyze_buying_intention(
        self, message: str, profile: CustomerProfile
    ) -> Dict[str, float]:
        ...
        # Sinais de compra
        buying_count = sum(
            1 for signal in self.buying_signals if signal in message_lower
        )
    ...
    def _generate_recommendations(
        self, score: float, factors: Dict[str, float], profile: CustomerProfile
    ) -> List[str]:
    ...
    def _estimate_value(
        self, profile: CustomerProfile, factors: Dict[str, float]
    ) -> float:
...
class LeadScoringService:
    ...
    def score_lead(
        ...
        # Log do resultado
        logger.info(
            f"Lead scored: {phone} - Score: {lead_score.total_score} - Category: {lead_score.category.value}"
        )

        return lead_score

    def _get_or_create_profile(
        self, phone: str, customer_data: Dict[str, Any] = None
    ) -> CustomerProfile:
    ...
    def _update_profile_with_interaction(self, profile: CustomerProfile, message: str):
        ...
        if any(word in message_lower for word in ["preço", "quanto", "valor"]):
            ...
            word in message_lower for word in ["problema", "reclamação", "insatisfeito"]
        ):
    ...
    def get_lead_analytics(self) -> Dict[str, Any]:
        ...
        scores = [
            data["last_score"].total_score for data in self.lead_database.values()
        ]
        categories = [
            data["last_score"].category.value for data in self.lead_database.values()
        ]

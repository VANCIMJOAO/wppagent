...
from app.models.database import (AdminUser, Appointment, Conversation,
...
class ReportExportService:
    ...
    async def export_appointments_report(
            ...
            data.append(
                {
                    ...
                    "Atualizado em": (
                        apt.updated_at.strftime("%d/%m/%Y %H:%M")
                        if apt.updated_at
                        else ""
                    ),
                }
        ...
        summary_data = {
            ...
            "status_breakdown": self._calculate_status_breakdown(
                [apt.status for apt in appointments]
            ),
        ...
        if format_type.lower() == "csv":
            ...
            return (
                await self._export_excel_appointments(data, summary_data),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        elif format_type.lower() == "pdf":
            return (
                await self._export_pdf_appointments(data, summary_data),
                "application/pdf",
            )
        else:
    ...
    async def export_conversations_report(
        ...
        summary_data = {
            ...
            "period": f"{date_from or 'Início'} até {date_to or 'Hoje'}",
            "status_breakdown": self._calculate_conversation_status_breakdown(
                [conv.status for conv in conversations]
            ),
            "generated_at": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
        }
        ...
        if format_type.lower() == "csv":
            ...
            return (
                await self._export_excel_conversations(data, summary_data),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        elif format_type.lower() == "pdf":
            return (
                await self._export_pdf_conversations(data, summary_data),
                "application/pdf",
            )
        else:
            raise ValueError(f"Formato não suportado: {format_type}")
    ...
    async def export_dashboard_report(
        ...
        async with AsyncSessionLocal() as session:
            ...
            if date_from:
                apt_query = apt_query.where(
                    func.date(Appointment.date_time) >= date_from
                )
            if date_to:
            ...
            if date_from:
                conv_query = conv_query.where(
                    func.date(Conversation.created_at) >= date_from
                )
            if date_to:
                conv_query = conv_query.where(
                    func.date(Conversation.created_at) <= date_to
                )
            ...
            if date_from:
                push_query = push_query.where(
                    func.date(PushNotification.sent_at) >= date_from
                )
            if date_to:
                push_query = push_query.where(
                    func.date(PushNotification.sent_at) <= date_to
                )
        ...
        if format_type.lower() == "excel":
            return (
                await self._export_excel_dashboard(metrics, daily_data),
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
        elif format_type.lower() == "pdf":
            return (
                await self._export_pdf_dashboard(metrics, daily_data),
                "application/pdf",
            )
        else:
    ...
    async def _export_excel_appointments(
        ...
        if data:
            ...
            for col_num, column_title in enumerate(df.columns, 1):
                ...
                cell.fill = PatternFill(
                    start_color="4a7cb8", end_color="4a7cb8", fill_type="solid"
                )
    ...
    async def _export_excel_conversations(
        self, data: List[Dict], summary: Dict
    ) -> bytes:
        ...
        if data:
            ...
            for column in data_ws.columns:
                ...
                data_ws.column_dimensions[column[0].column_letter].width = min(
                    max_length + 2, 50
                )
    ...
    async def _export_excel_dashboard(
        self, metrics: Dict, daily_data: List[Dict]
    ) -> bytes:
        ...
        metrics_ws["A1"].fill = PatternFill(
            start_color="366092", end_color="366092", fill_type="solid"
        )
        ...
        if daily_data:
            ...
            if len(daily_data) > 1:
                ...
                data_ref = Reference(
                    ...
                    min_col=2,
                    min_row=1,
                    max_row=len(daily_data) + 1,
                    max_col=2,
                )
                cats_ref = Reference(
                    daily_ws, min_col=1, min_row=2, max_row=len(daily_data) + 1
                )
    ...
    async def _export_pdf_appointments(self, data: List[Dict], summary: Dict) -> bytes:
        ...
        if data:
            elements.append(
                Paragraph("DETALHAMENTO DOS AGENDAMENTOS", styles["Heading2"])
            )
            ...
            if len(data) > 50:
                ...
                elements.append(
                    Paragraph(
                        f"<i>Mostrando primeiros 50 de {len(data)} registros</i>",
                        styles["Normal"],
                    )
                )
    ...
    async def _export_pdf_dashboard(
        self, metrics: Dict, daily_data: List[Dict]
    ) -> bytes:
        ...
        for key, value in metrics.items():
            if key not in ["período", "gerado_em"]:
                metrics_table_data.append([key.replace("_", " ").title(), str(value)])
        ...
        elements.append(
            Paragraph(
                f"<b>Período analisado:</b> {metrics.get('período', 'N/A')}",
                styles["Normal"],
            )
        )
        elements.append(
            Paragraph(
                f"<b>Relatório gerado em:</b> {metrics.get('gerado_em', 'N/A')}",
                styles["Normal"],
            )
        )

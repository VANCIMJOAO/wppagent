...
class OptimizedQueryServicePD001:
    ...
    async def get_conversations_optimized(
        ...
        query = select(Conversation).options(
            ...
            selectinload(Conversation.messages).options(
                joinedload(Message.user)    # Precarregar users das mensagens também
    ...
    async def get_conversations_with_counts_batch(
        ...
        message_count_subquery = select(func.count(Message.id)).where(
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            for line in explain_output:
                if "actual time=" in line:
                    ...
                    if total_time is None:
                        import re
                        time_match = re.search(r'actual time=[\d.]+\.\.[\d.]+ rows=\d+ loops=\d+', line)
                        if time_match:
                            total_time = line
            ...
            analysis = {
                ...
                "seq_scans": seq_scans,
                "performance_grade": "A" if seq_scans == 0 and index_scans > 0 else "B" if seq_scans == 0 else "C"
            }
    ...
    async def benchmark_before_after_optimization(
        ...
        optimized_conversations = await OptimizedQueryServicePD001.get_conversations_optimized(
            session, limit=10
        )

    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if secrets_dir.exists() and secrets_dir.is_dir():
                logger.info(
                    f"✅ Docker Secrets disponível em: {self.config.secrets_path}"
                )
                ...
                logger.warning(
                    f"⚠️ Docker Secrets não disponível em: {self.config.secrets_path}"
                )
    ...
    def list_available_secrets(self) -> list:
        ...
        if secrets_dir.exists():
            for secret_file in secrets_dir.iterdir():
                if (
                    secret_file.is_file()
                    and secret_file.name.startswith(self.config.prefix)
                    and secret_file.name not in secrets
                ):

...
class AlertManager:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                try:
                    ...
                    logger.warning(
                        f"Tabela de mensagens não disponível, usando dados simulados: {e}"
                    )
    ...
    async def _mock_message_stats(self) -> Dict:
        ...
        return {"sent": sent, "failed": failed, "total": sent + random.randint(0, 50)}
...
async def monitoring_task():
    ...
    while True:
        try:
            ...
            if summary["total"] > 0:
                logger.info(
                    f"📊 Alertas ativos: {summary['total']} "

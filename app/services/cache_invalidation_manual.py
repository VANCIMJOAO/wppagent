...
class CacheInvalidationManager:
    ...
    async def invalidate_by_patterns(self, patterns: List[str], dry_run: bool = False) -> Dict:
    ...
    async def invalidate_by_scopes(self, scopes: List[CacheScope], dry_run: bool = False) -> Dict:
    ...
    async def invalidate_with_cascade(self, keys: List[str], dry_run: bool = False) -> Dict:
            ...
            for key in keys:
                for dependency in rule.dependencies:
                    if re.match(dependency.replace('*', '.*'), key):
                        ...
                        dependent_keys = await self.get_cache_keys_by_pattern(rule.pattern)
    ...
    async def execute_manual_invalidation(self, request: InvalidationRequest) -> InvalidationResponse:
        ...
        try:
            ...
            if request.patterns:
                result = await self.invalidate_by_patterns(request.patterns, request.dry_run)
                all_invalidated_keys.extend(result["invalidated_keys"])
                
            # Invalidação por escopos
            if request.scopes:
                result = await self.invalidate_by_scopes(request.scopes, request.dry_run)
                all_invalidated_keys.extend(result["invalidated_keys"])

...
class AdvancedRateLimiter:
    ...
    def check_rate_limit(
        self, ip: str, endpoint: str, user_id: str = None
    ) -> RateLimitResult:
        ...
        if self._is_ip_blocked(ip):
            return RateLimitResult(
                ...
                + timedelta(seconds=self.blocked_ips.get(ip, 300)),
    ...
    def _check_sliding_window(
        self, ip: str, endpoint: str, user_id: str, config: RateLimitConfig
    ) -> RateLimitResult:
        ...
        self.storage.set(
            key,
            {"requests": list(requests), "violations": data.get("violations", 0)},
    ...
    def _check_token_bucket(
        ...
        # Atualizar tokens
        current_tokens = min(config.requests_per_window, data["tokens"] + tokens_to_add)
        ...
        self.storage.set(
            key,
            {"tokens": current_tokens, "last_refill": now},
            ttl=config.window_size * 2,
        )
    ...
    def _record_violation(self, ip: str, endpoint: str, reason: str):
        ...
        self.violation_tracker[ip].append(
            {"timestamp": now, "endpoint": endpoint, "reason": reason}
        )

        # Limpar violações antigas (> 24 horas)
        cutoff = now - timedelta(days=1)
        self.violation_tracker[ip] = [
            v for v in self.violation_tracker[ip] if v["timestamp"] > cutoff
        ]
        ...
        recent_violations = [
            v
            for v in self.violation_tracker[ip]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_violations = sum(
            len(violations) for violations in self.violation_tracker.values()
        )
        ...
        return {
            ...
            "violation_by_ip": {
                ip: len(violations) for ip, violations in self.violation_tracker.items()
            },
            "configs": {
                endpoint: asdict(config) for endpoint, config in self.configs.items()
            },
        }

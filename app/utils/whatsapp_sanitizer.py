...
class WhatsAppSanitizer:
    ...
    def sanitize_phone_number(cls, phone: str) -> str:
        ...
        try:
            ...
            clean_phone = clean_phone.replace("@c.us", "").replace(
                "@s.whatsapp.net", ""
            )
    ...
    def validate_webhook_signature(
        cls, signature: str, payload: str, webhook_secret: str
    ) -> bool:
        """
    ...
    def _sanitize_text_message(cls, text: str) -> str:
        ...
        if not re.match(cls.PATTERNS["safe_text"], text):
            ...
            text = re.sub(
                r'[^\w\s\u00C0-\u024F\u1E00-\u1EFF\U0001F000-\U0001F9FF\U00002000-\U000027BF\U0000FE00-\U0000FE0F.,!?:;\-_@#$%&*()\[\]{}="\'\/\\+\n\r\t•]',
                "",
                text,
            )
...
def validate_security(
    content: str, filename: str = None, mime_type: str = None
) -> Dict[str, bool]:
    ...
    return {
        ...
        "is_malware": security_validator.is_potential_malware(
            filename or "", mime_type or ""
        ),
    }

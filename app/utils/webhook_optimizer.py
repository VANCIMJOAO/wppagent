...
from app.services.structured_apm import LogCategory, get_structured_logger
...
class BatchProcessor:
    ...
    async def process_messages_optimized(
        self, messages: List[Dict[str, Any]], db: AsyncSession
        ...
        try:
            ...
            return (
                0,
                batch_size,
                {"error": "timeout", "processing_time": self.batch_timeout},
            )
        ...
        metrics = {
            ...
            "messages_per_second": (
                round(batch_size / processing_time, 2) if processing_time > 0 else 0
            ),
...
class BulkDatabaseOptimizer:
    ...
    @staticmethod
    async def bulk_user_lookup(db: AsyncSession, wa_ids: List[str]) -> Dict[str, Any]:
        """

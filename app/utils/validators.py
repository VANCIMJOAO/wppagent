...
class RobustValidator:
    ...
    def validate_datetime(
        value: Any, field_name: str, required: bool = True, future_only: bool = False
    ) -> datetime:
        """Validar data/hora"""
        if not required and (value is None or value == ""):
            return None
        ...
        if isinstance(value, datetime):
            ...
            try:
                ...
                raise ValidationError(
                    f"❌ {field_name} deve ser uma data/hora válida. Erro: {str(e)}"
                )
    ...
    def validate_status(value: Any, status_type: str) -> str:
        ...
        if status_str not in valid_statuses:
            raise ValidationError(
                f"❌ Status inválido. Valores aceitos: {', '.join(valid_statuses)}"
            )
    ...
    def validate_sql_query(query: str) -> str:
        ...
        for pattern in forbidden_patterns:
            if re.search(pattern, query_upper):
                ...
                raise ValidationError(
                    f"❌ Query contém operação proibida: {match.group()}"
                )
...
def validate_user_input(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Email opcional
        if "email" in data:
            validated["email"] = RobustValidator.validate_email(
        ...
        for field in ["user_id", "conversation_id"]:
            if field in data:
                validated[field] = RobustValidator.validate_integer(
                    data[field], f"ID {field}", min_value=1
                )
    ...
    try:
        ...
        if "date_time" in data:
            validated["date_time"] = RobustValidator.validate_datetime(
                data["date_time"], "Data/Hora do agendamento", future_only=True
            )

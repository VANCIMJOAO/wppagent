...
@callback(
    [Output("user-form-feedback", "children"), Output("user-form-feedback", "color")],
...
def handle_user_form_submission(n_clicks, name, phone, email):
    ...
    try:
        ...
        raw_data = {"nome": name, "telefone": phone, "email": email}
...
@callback(
    ...
    [State("search-input", "value"), State("search-table-dropdown", "value")],
)
    ...
    try:
        ...
        if validated_table == "users":
            ...
            raise ValidationError(
                f"Busca não implementada para tabela: {validated_table}"
            )
...
@callback(
    [Output("data-table", "data"), Output("pagination-info", "children")],
    [Input("page-input", "value"), Input("per-page-dropdown", "value")],
)
def handle_pagination(page, per_page):
    ...
    try:
        ...
        data_result = safe_execute_query(
            query, {"limit": valid_per_page, "offset": offset}
        )
...
def handle_date_filter(n_clicks, start_date, end_date):
    ...
    try:
        ...
        result = safe_execute_query(
            query, {"start_date": valid_start, "end_date": valid_end}
        )
        ...
        if not rows:
            return dbc.Alert(
                "Nenhum dado encontrado no período selecionado.", color="info"
            )
        ...
        return [
            html.H5(
                f"Dados de {valid_start.strftime('%d/%m/%Y')} a {valid_end.strftime('%d/%m/%Y')}"
            ),
        ...
        if errors:
            error_summary = html.Div(
                [
                    ...
                    html.Ul(
                        [html.Li(error) for error in errors[:10]]
                    ),  # Mostrar apenas 10 primeiros
                ]
            )
            ...
            [
                dbc.CardBody(
                    [
                        ...
                        html.Hr(),
                        html.P(
                            "Registros válidos foram processados com sucesso!"
                            if success_count > 0
                            else "Nenhum registro válido encontrado."
                        ),
                    ]
                )
            ]

...
class LoggerSetup:
    ...
    def _setup_file_handlers(
        self, formatter: logging.Formatter, log_level: int
    ) -> None:
    ...
    def _performance_filter(self, record: logging.LogRecord) -> bool:
        ...
        return hasattr(record, "extra_data") and record.extra_data.get(
    ...
    def _business_filter(self, record: logging.LogRecord) -> bool:
        ...
        return any(
            business_logger in record.name.lower()
            for business_logger in business_loggers
        )
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                duration = (time.time() - start_time) * 1000
                logger.error(
                    f"Erro em {name} após {duration:.2f}ms: {e}",
                    {"operation": name, "duration_ms": duration},
                    exc_info=True,
                )
                raise
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.error(
                    f"Erro na função {func_name}: {e}", log_data, exc_info=True
                )
                raise
...
def safe_print(
    *args, level: LogLevel = LogLevel.INFO, logger_name: str = "app", **kwargs
):
    """

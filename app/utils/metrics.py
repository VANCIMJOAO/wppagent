...
process_resident_memory_bytes = Gauge(
    "process_resident_memory_bytes", "Resident memory size in bytes", registry=registry
)
...
rate_limit_hits_total = Counter(
    "rate_limit_hits_total", "Total rate limit hits", ["limit_type"], registry=registry
)
...
cache_hit_rate = Gauge(
    "cache_hit_rate", "Cache hit rate as a percentage", registry=registry
)
...
# Application Info
app_info = Info(
    "whatsapp_agent_info", "WhatsApp Agent application information", registry=registry
)
...
def http_request_timer(method: str, endpoint: str):
    ...
    return timer(
        lambda duration: metrics_collector.record_http_request(
...
def webhook_timer():
    ...
    return timer(
        lambda duration: metrics_collector.record_webhook_processing(
            "success", duration
        )
    )
...
def database_timer(operation: str):
    ...
    return timer(
        lambda duration: metrics_collector.record_database_query(operation, duration)
    )
...
def lead_scoring_timer():
    ...
    )  # Duration handled separately in record_lead_scoring

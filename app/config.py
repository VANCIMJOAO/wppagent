...
class CompatibilitySettings:
    ...
    def __getattr__(self, name):
        ...
        mapping = {
            "meta_access_token": lambda: (
                self._config.meta_access_token.get_secret_value()
                if self._config.meta_access_token
                else None
            ),
            ...
            "webhook_verify_token": lambda: (
                self._config.webhook_verify_token.get_secret_value()
                if self._config.webhook_verify_token
                else None
            ),
            "admin_password": lambda: (
            ...
            "whatsapp_webhook_secret": lambda: (
                self._config.whatsapp_webhook_secret.get_secret_value()
                if self._config.whatsapp_webhook_secret
                else None
            ),
            "database_dsn": lambda: self._config.database_url,
            # VAPID configuration
            "VAPID_PRIVATE_KEY": lambda: (
                self._config.vapid_private_key.get_secret_value()
                if self._config.vapid_private_key
                else None
            ),

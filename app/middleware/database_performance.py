...
class DatabasePerformanceMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        self._log_performance_metrics(
            request, response, total_duration, query_count, total_query_time, queries
        )
    ...
    def _log_performance_metrics(
        ...
        log_data = {
            ...
            "query_efficiency": (
                round((total_query_time / total_duration) * 100, 1)
                if total_duration > 0
                else 0
            ),
        ...
        if total_duration <= 0.5 and query_count <= 10:
            logger.info(
                f"✅ Fast request: {request.method} {request.url.path}", extra=log_data
            )

...
class UserRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, redis_client=None):
        ...
        if redis_client:
            ...
            try:
                ...
                logger.error(f"❌ Redis connection failed: {e}")
                logger.warning(
                    "⚠️ Rate limiting will be disabled due to Redis connection failure"
                )
                self.redis = None
        ...
        logger.info(
            "UserRateLimitMiddleware initialized with Redis and flexible endpoint limits"
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if not user_info:
            logger.debug(
                f"No user info, handling IP rate limiting for {request.url.path}"
            )
        ...
        try:
            ...
            if response:
                ...
                await self._add_rate_limit_headers(
                    response, user_id, endpoint_key, limit_config
                )
            ...
            logger.warning(
                "Rate limiting disabled due to Redis error - allowing request"
            )
            ...
            try:
                ...
                if response:
                    ...
                    logger.error(
                        f"No response received in error handler for user {user_id}"
                    )
                    from fastapi.responses import JSONResponse
                ...
                logger.error(
                    f"Final error handler failed for user {user_id}: {final_e}"
                )
                from fastapi.responses import JSONResponse

                return JSONResponse(
                    status_code=500, content={"error": "Critical middleware error"}
                )
    ...
    async def _handle_ip_rate_limiting(self, request: Request, call_next):
        ...
        try:
            ...
            try:
                rate_limit_result = await self._check_rate_limit(
                    f"ip:{client_ip}", endpoint_key, limit_config
                )
                ...
                response = await call_next(request)
                await self._add_rate_limit_headers(
                    response, f"ip:{client_ip}", endpoint_key, limit_config
                )
            ...
            try:
                ...
                return JSONResponse(
                    status_code=500, content={"error": "Internal rate limiting error"}
                )
    ...
    async def _check_rate_limit(
        self, user_id: str, endpoint_key: str, config: Dict
    ) -> Dict:
        ...
        try:
            ...
            pipe.zremrangebyscore(
                burst_key, 0, current_time - 60
            )  # Burst window de 1 minuto
            ...
            if current_requests >= config["requests"]:
                logger.warning(
                    f"Rate limit exceeded for {user_id} on {endpoint_key}: {current_requests}/{config['requests']}"
                )
                return {
                    "exceeded": True,
            ...
            if burst_requests >= config.get("burst", 10):
                logger.warning(
                    f"Burst limit exceeded for {user_id} on {endpoint_key}: {burst_requests}/{config.get('burst', 10)}"
                )
                return {
                    "exceeded": True,
    ...
    async def _handle_rate_limit_exceeded(
        self, rate_limit_result: Dict, config: Dict
    ) -> Response:
    ...
    async def _add_rate_limit_headers(
        self, response: Response, user_id: str, endpoint_key: str, config: Dict
    ):
        ...
        try:
            ...
            current_count = await self.redis.zcount(
                main_key, window_start, current_time
            )
            ...
            if "burst" in config:
                burst_key = f"rate_limit:burst:user:{user_id}:{endpoint_key}"
                burst_current = await self.redis.zcount(
    ...
    async def get_user_rate_limit_status(
        ...
        try:
            ...
            for ep in endpoints:
                config = self._get_limit_config(
                    ep, "regular"
                )  # Usar tipo regular como padrão
    ...
    async def reset_user_rate_limit(self, user_id: str, endpoint: str = None):
        ...
        try:
            ...
            if keys:
                ...
                logger.info(
                    f"Reset rate limit for user {user_id}, endpoint: {endpoint or 'all'}"
                )

...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enabled: bool = True):
        super().__init__(app)
        self.enabled = (
            enabled and rate_limiter is not None and whatsapp_rate_limiter is not None
        )

        if not self.enabled:
            logger.warning(
                "Rate limiting middleware disabled due to missing dependencies"
            )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if not route_config or "limiter" not in route_config:
                logger.warning(
            ...
            if response is None:
                logger.error(f"No response received for {request.url.path}")
                return JSONResponse(
                    status_code=500,
                    content={
                        "error": "no_response",
            ...
            if limit_info and "remaining_requests" in limit_info:
            ...
            logger.info(
                ...
                f"(remaining: {limit_info.get('remaining_requests', 'unknown') if limit_info else 'unknown'})"
            )

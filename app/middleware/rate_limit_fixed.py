...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, enabled: bool = True):
        ...
        if not self.enabled:
            logger.warning(
                "Rate limiting middleware disabled due to missing dependencies or explicit disable"
            )
            ...
            for route_pattern, config in self.route_configs.items():
                if path.startswith(route_pattern):
                    ...
                    if (
                        ...
                        and config["limiter"] is not None
                    ):
                        return config
                    else:
                        logger.warning(
                            f"Invalid config for route {route_pattern}: {config}"
                        )
    ...
    def _should_skip_rate_limit(self, request: Request) -> bool:
        ...
        try:
            ...
            if (
                ...
                and request.headers.get("X-Performance-Test") == "true"
            ):
                return True
    ...
    def _is_internal_request(self, request: Request) -> bool:
        ...
        try:
            ...
            return any(
                client_ip.startswith(range_start) for range_start in private_ranges
            )
        except Exception as e:
    ...
    async def dispatch(
        ...
        try:
            ...
            if not route_config:
                logger.debug(
                    f"No rate limit config for {request.url.path}, allowing request"
                )
                response = await call_next(request)
                if response is None:
                    logger.error(f"call_next returned None for {request.url.path}")
            ...
            if not hasattr(limiter, "is_allowed"):
                logger.warning(
                    f"Invalid limiter for {request.url.path}, allowing request"
                )
                response = await call_next(request)
                if response is None:
                    logger.error(f"call_next returned None for {request.url.path}")
            ...
            try:
                result = await limiter.is_allowed(
                    client_id=client_id, endpoint=endpoint, cost=1
                )
                ...
                if response is None:
                    logger.error(
                        f"call_next returned None after limiter error for {request.url.path}"
                    )
                    return JSONResponse(
                        status_code=500,
                        content={
            ...
            if not is_allowed:
                ...
                return JSONResponse(
                    ...
                    content={
                        ...
                        "message": (
                            limit_info.get("message", "Too many requests")
                            if limit_info
                            else "Too many requests"
                        ),
                        "retry_after": (
                            limit_info.get("retry_after", 60) if limit_info else 60
                        ),
                        "details": limit_info,
                    },
                    headers={
                        ...
                        "X-RateLimit-Limit": str(
                            limit_info.get("max_requests", "unknown")
                            if limit_info
                            else "unknown"
                        ),
                        "X-RateLimit-Remaining": "0",
                        "X-RateLimit-Reset": str(
                            limit_info.get("window_reset", int(time.time() + 60))
                            if limit_info
                ...
                try:
                    if "remaining_requests" in limit_info:
                        ...
                        response.headers["X-RateLimit-Remaining"] = str(
                            limit_info["remaining_requests"]
                        )
                        response.headers["X-RateLimit-Reset"] = str(
                            limit_info.get("window_reset", "unknown")
                        )
            ...
            try:
                ...
                remaining = (
                    limit_info.get("remaining_requests", "unknown")
                    if limit_info
                    else "unknown"
                )
                logger.info(
            ...
            try:
                ...
                if response is None:
                    logger.error(
                        f"call_next returned None after middleware error for {request.url.path}"
                    )
                    return JSONResponse(
                        status_code=500,
                        content={
                            "error": "no_response",
                            "message": "No response generated after middleware error",
                        },
                    )
                return response
            except Exception as call_error:
                logger.error(
                    f"Failed to process request after middleware error: {call_error}"
                )
                return JSONResponse(
                    status_code=500,
                    content={

        ...
        try:
            ...
            if client_id in self.requests_count:
                self.requests_count[client_id]["requests"] = [
                    req_time
                    for req_time in self.requests_count[client_id]["requests"]
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not self.enabled or self._should_skip(request):
            try:
                ...
                return JSONResponse(
                    status_code=500, content={"error": "request_processing_failed"}
                )
        ...
        try:
            ...
            if limit_info:
                response.headers["X-RateLimit-Remaining"] = str(
                    limit_info.get("remaining", "unknown")
                )
            ...
            try:
                ...
                return JSONResponse(
                    status_code=500, content={"error": "middleware_error"}
                )

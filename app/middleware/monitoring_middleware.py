...
class MonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.error(
                ...
                {"error": str(e), "response_time_ms": round(response_time_ms, 2)},
                exc_info=True,
            )
    ...
    async def _record_business_metrics(
        self, request: Request, response: Response, user_id: str
    ):
...
def setup_monitoring_middlewares(app):
    ...
    slow_threshold = (
        config.alert_response_time_threshold_ms * 0.8
    )  # 80% do threshold de alerta

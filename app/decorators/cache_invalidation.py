        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                await cache_invalidation_service.invalidate_for_event(
                    event=event, context=context if context else None
                )

                logger.debug(
                    f"✅ Cache invalidated automatically - Event: {event.value}, Context: {context}"
                )
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            for event_config in events_and_configs:
                ...
                task = _invalidate_for_event_with_config(event, result, kwargs, config)
...
async def _invalidate_for_event_with_config(
    event: CacheEvent, result: Any, func_kwargs: Dict[str, Any], config: Dict[str, Any]
):
...
def invalidate_appointment_cache_on_success(event: CacheEvent):
    ...
    return invalidate_cache(
        event, entity_id_param="appointment_id", extract_from_result=True
    )
...
def invalidate_conversation_cache_on_success(event: CacheEvent):
    ...
    return invalidate_cache(
        event, entity_id_param="conversation_id", extract_from_result=True
    )
...
def invalidate_client_cache_on_success(event: CacheEvent):
    ...
    return invalidate_cache(
        event, entity_id_param="client_id", extract_from_result=True
    )

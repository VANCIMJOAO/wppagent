...
async def get_clients(
    ...
    search: Optional[str] = Query(
        None, description="Busca por nome, telefone ou email"
    ),
    ...
    try:
        ...
        query = select(User).where(
            and_(
                User.nome.is_not(None), User.nome != "", ~User.nome.like("%[DELETED]%")
            )
        )
...
async def get_monthly_stats(
    ...
    try:
        ...
        result = await db.execute(
            monthly_query, {"start_date": start_date, "limit_months": months}
        )
...
async def get_daily_stats(
    ...
    try:
        ...
        total_clients_query = select(func.count(User.id)).where(
            and_(User.nome.isnot(None), User.nome != "", ~User.nome.like("%[DELETED]%"))
        )
...
async def export_report(
    ...
    try:
        ...
        logger.info(
            f"Exportando {len(data)} registros do tipo '{type}' em formato '{format}'"
        )
...
async def get_recent_activity(
    limit: int = Query(
        10, ge=1, le=50, description="Número de atividades para retornar"
    ),
    db: AsyncSession = Depends(get_db),
    current_admin: AdminUser = Depends(get_current_admin_user),
):
...
async def get_dashboard_appointments(
    ...
    try:
        ...
        query = select(
            ...
            Appointment.__table__.join(User, Appointment.user_id == User.id).outerjoin(

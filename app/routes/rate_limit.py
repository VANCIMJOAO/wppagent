...
async def reset_rate_limit(
    ...
    try:
        ...
        logger.info(
            f"Rate limit reset by admin {admin_user.username} for user {user_id}, endpoint: {endpoint or 'all'}"
        )
...
async def get_rate_limit_stats(
    ...
    try:
        ...
        stats = {
            ...
            "violation_types": {"main_limit": 0, "burst_limit": 0},
...
async def update_rate_limit_config(
    config: RateLimitConfig, admin_user: AdminUser = Depends(get_current_admin_user)
):
    """
    ...
    try:
        ...
        if config.requests <= 0 or config.window <= 0:
            raise HTTPException(
                status_code=400, detail="Requests and window must be positive"
            )
        ...
        new_config = {"requests": config.requests, "window": config.window}
        ...
        logger.info(
            f"Rate limit config updated by admin {admin_user.username} for endpoint {config.endpoint}"
        )
...
async def update_user_rate_limit(
    update: UserRateLimitUpdate, admin_user: AdminUser = Depends(get_current_admin_user)
):
    """
    ...
    try:
        ...
        logger.info(
            f"User rate limit update requested by admin {admin_user.username} for user {update.user_id}"
        )
...
async def test_rate_limit(
    ...
    try:
        ...
        return {
            ...
            "results": results,
            "final_status": await rate_limiter.get_user_rate_limit_status(
                user_id, endpoint
            ),
            "timestamp": datetime.now().isoformat(),

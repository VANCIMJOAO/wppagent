...
async def get_rate_limit_stats(
    ...
    try:
        ...
        if not middleware:
            raise HTTPException(
                status_code=503,
                detail="Webhook rate limiting middleware not available"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting rate limit stats: {str(e)}"
        )
...
async def clear_rate_limit_blocks(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error clearing rate limit blocks: {str(e)}"
        )
...
async def test_h003_compliance(
    ...
    requests_count: int = Query(101, ge=101, le=150, description="Número de requests (deve ser > 100)"),
    ...
    try:
        ...
        h003_compliance = (
            first_block_at_request is not None and
            first_block_at_request <= 101 and  # Deve bloquear até request 101
            blocked_requests > 0 and
            successful_requests <= 100  # Máx 100 requests bem-sucedidas
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"H003 test failed: {str(e)}"
        )
...
async def test_health_check_exemption(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Health check test failed: {str(e)}"
        )

...
from app.services.structured_apm import (
    get_structured_logger, LogCategory, LogLevel,
    get_current_context
)
                ...
                async with aiofiles.open(log_file, 'r', encoding='utf-8') as f:
                    ...
                    for line in reversed(lines[-1000:]):  # Últimas 1000 linhas
                        ...
                        try:
                            ...
                            if service_filter and log_entry.get("service") != service_filter:
                                continue
    ...
    async def get_performance_metrics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for log in logs:
            ...
            if perf_metrics:
                ...
                performance_data["operations_by_type"][operation_name].append(duration_ms)
        ...
        if durations:
            ...
            performance_data["slowest_operations"] = performance_data["slowest_operations"][:10]
    ...
    try:
        ...
        dashboard = {
            ...
            "system_health": {
                ...
                "status": "healthy" if error_data.get("error_rate_per_hour", 0) < 5 else "degraded"
            },
...
async def get_recent_logs(
    ...
    level: Optional[str] = Query(None, description="Filtro por nível (DEBUG, INFO, WARNING, ERROR, CRITICAL)"),
    ...
    try:
        ...
        if operation_filter:
            ...
            metrics["slowest_operations"] = [
                op for op in metrics["slowest_operations"]
    ...
    try:
        ...
        if error_type:
            analysis["top_error_types"] = [
                error for error in analysis["top_error_types"]
                if error_type.lower() in error["type"].lower()
            ]
            
            analysis["recent_errors"] = [
                error for error in analysis["recent_errors"]
...
async def get_current_apm_context(
    current_admin: dict = Depends(require_admin)
):
    ...
    try:
        ...
        if format == "json":
            ...
            return StreamingResponse(
                ...
                headers={"Content-Disposition": f"attachment; filename=logs_{hours}h_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"}
            )
            ...
            writer.writerow([
                ...
                "request_id", "user_id", "operation", "logger_name"
            ])
            ...
            return StreamingResponse(
                ...
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename=logs_{hours}h_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"}
            )

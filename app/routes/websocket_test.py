...
async def test_websocket_broadcast(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao enviar evento de teste: {str(e)}"
        )
    ...
    try:
        ...
        for item in events_to_send:
            ...
            results.append(
                {
                    "topic": item["topic"],
                    "connections": websocket_manager.get_topic_connections_count(
                        item["topic"]
                    ),
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao simular evento de agendamento: {str(e)}"
        )


@router.post("/test-dashboard-stats")
async def test_dashboard_stats_update(current_user: Dict = Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao atualizar stats: {str(e)}"
        )

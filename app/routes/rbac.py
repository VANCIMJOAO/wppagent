...
from app.auth.rbac_decorators import (
    RequirePermission, RequireRole, RequireSuperAdmin,
    get_current_user
)
...
async def delete_role(
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Erro ao deletar role ou role não pode ser deletado")
...
async def list_permissions(
    ...
    return {
        ...
        "categories": list(set(def_.category for def_ in PERMISSION_DEFINITIONS.values())),
        "risk_levels": list(set(def_.risk_level for def_ in PERMISSION_DEFINITIONS.values()))
    }
...
async def check_multiple_permissions(
    ...
    if request.check_type == "any":
        ...
        has_permissions = user.has_all_permissions(request.permissions) if user else False
...
async def get_system_roles_info(
    ...
    return {
        "system_roles": roles_info,
        "total_system_roles": len(ROLE_CONFIGURATIONS)
    }
    ...
    for user_id in user_ids:
        ...
        if user:
            ...
            if role_type not in current_roles:
                ...
                results.append({"user_id": user_id, "success": True, "already_has": True})
        else:

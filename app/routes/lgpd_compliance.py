...
class AccountDeletionRequest(BaseModel):
    ...
    user_identifier: str = Field(..., description="Telefone, email ou ID do usuário")
    confirmation: bool = Field(..., description="Confirmação da operação (deve ser True)")
...
async def request_data_portability(
    ...
    try:
        ...
        if current_user.phone != request.user_identifier and current_user.email != request.user_identifier:
...
async def delete_user_account(
    ...
    try:
        ...
        if not request.confirmation:
            raise HTTPException(status_code=400, detail="Confirmação obrigatória não fornecida")
        
        # Verificar se o usuário pode deletar esses dados
        if current_user.phone != request.user_identifier and current_user.email != request.user_identifier:
            raise HTTPException(status_code=403, detail="Acesso negado para eliminação desses dados")
...
async def apply_retention_policies(
    ...
    try:
        ...
        logger.info(f"✅ Políticas aplicadas: {response.total_records_processed} registros processados")
        return response

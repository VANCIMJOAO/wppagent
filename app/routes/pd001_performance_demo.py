...
async def conversations_before_pd001(
    ...
    try:
        ...
        logger.error(
            f"❌ PD001 BEFORE demo failed: {str(e)}", category="performance_demo"
        )
...
async def conversations_after_pd001(
    ...
    try:
        ...
        conversations = await OptimizedQueryServicePD001.get_conversations_optimized(
            session=db, limit=limit, offset=offset, user_id=user_id, status=status
        )
        ...
        logger.error(
            f"❌ PD001 AFTER demo failed: {str(e)}", category="performance_demo"
        )
        raise HTTPException(status_code=500, detail=f"Demo failed: {str(e)}")
...
async def appointments_optimized_pd001(
    ...
    try:
            ...
            result_data.append(
                {
                    ...
                    "service": {
                        ...
                        "price": (
                            float(appt.service.preco)
                            if appt.service and appt.service.preco
                            else None
                        ),
                    },
                    "business": {
        ...
        logger.error(
            f"❌ PD001 Appointments demo failed: {str(e)}", category="performance_demo"
        )
        raise HTTPException(status_code=500, detail=f"Demo failed: {str(e)}")


@router.get("/conversations/benchmark")
async def conversations_benchmark_pd001(db: AsyncSession = Depends(get_db)):
    """
...
async def query_analysis_pd001(query_type: str, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        if query_type not in queries:
            raise HTTPException(
                status_code=400, detail=f"Query type '{query_type}' not found"
            )

        analysis = await OptimizedQueryServicePD001.analyze_query_performance(
            session=db, query_name=query_type, raw_sql=queries[query_type]
        )
        ...
        logger.error(
            f"❌ PD001 Query analysis failed: {str(e)}", category="performance_demo"
        )
...
async def conversations_batch_counts_pd001(
    ...
    try:
        conversations_data = (
            await OptimizedQueryServicePD001.get_conversations_with_counts_batch(
                session=db, limit=limit, offset=offset
            )
        )
            ...
            result.append(
                {
                    ...
                    "last_message_at": (
                        conv_data["conversation"].last_message_at.isoformat()
                        if conv_data["conversation"].last_message_at
                        else None
                    ),
        ...
        logger.error(
            f"❌ PD001 Batch demo failed: {str(e)}", category="performance_demo"
        )

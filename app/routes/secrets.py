...
async def create_secret(
    request: CreateSecretRequest, user: Dict = Depends(require_admin)
):
    """Criar novo secret"""
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid secret type. Valid types: {valid_types}"
        )
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail=f"Secret {request.secret_id} already exists"
        )

    secret = secrets_manager.create_secret(
        request.secret_id, secret_type, request.value, request.metadata or {}
    )
...
async def list_secrets(
    secret_type: Optional[str] = None, user: Dict = Depends(require_admin)
):
...
async def get_secret_info(
    secret_id: str, version: Optional[int] = None, user: Dict = Depends(require_admin)
):
...
async def get_secret_value(
    ...
    secrets_manager._log_audit(
        ...
        secret_id,
        {"accessed_by": user["user_id"], "version": secret.version},
    )
...
async def rotate_secret(
    ...
    background_tasks.add_task(
        _notify_secret_rotation, secret_id, secret.version, new_secret.version
    )
    ...
    return {
        ...
        "expires_at": (
            new_secret.expires_at.isoformat() if new_secret.expires_at else None
        ),
    }
...
async def revoke_secret(
    ...
    if not success:
        raise HTTPException(
            status_code=404, detail=f"Secret {secret_id} not found or already revoked"
        )

    if version:
        return {"message": f"Secret {secret_id} version {version} revoked successfully"}
    else:
        return {"message": f"All versions of secret {secret_id} revoked successfully"}


@router.get("/expiring/check")
async def check_expiring_secrets(
    days_ahead: int = 7, user: Dict = Depends(require_admin)
):
...
async def auto_rotate_expired(
    background_tasks: BackgroundTasks, user: Dict = Depends(require_admin)
):
...
async def get_secret_audit_log(
    secret_id: str, limit: int = 50, user: Dict = Depends(require_admin)
):
    """Obter log de auditoria de um secret"""
    audit_log = secrets_manager.get_audit_log(secret_id, limit)

    return {"secret_id": secret_id, "audit_events": audit_log}


@router.get("/audit/all")
async def get_all_audit_logs(limit: int = 100, user: Dict = Depends(require_admin)):
    """Obter todos os logs de auditoria"""
    audit_log = secrets_manager.get_audit_log(None, limit)

    return {"audit_events": audit_log, "total_events": len(audit_log)}


@router.post("/emergency/revoke-all")
async def emergency_revoke_all(confirm: str, user: Dict = Depends(require_admin)):
...
# Funções auxiliares
async def _notify_secret_rotation(
    secret_id: str, old_version: Optional[int], new_version: Optional[int]
):

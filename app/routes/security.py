    ...
    try:
        ...
        for key, value in test_data.items():
            try:
                ...
                # Descriptografar
                decrypted = encryption_service.decrypt_to_string(
                    encrypted, f"test_{key}"
                )
                ...
                results[key] = {"test_passed": False, "error": str(e)}

        # Calcular resultado geral
        all_passed = all(
            result.get("test_passed", False) for result in results.values()
        )
...
async def test_sensitive_data_encryption():
    ...
    try:
        ...
        sensitive_encrypted = sum(
            1
            for k, v in encrypted_env.items()
...
async def check_https_headers(request: Request):
    ...
    try:
        ...
        is_https = (
            request.url.scheme == "https" or headers.get("x-forwarded-proto") == "https"
        )
        ...
        return {
            ...
            "security_score": round(
                (len(present_headers) / len(security_headers)) * 100
            ),
            "timestamp": datetime.utcnow().isoformat(),
        }
...
async def get_security_compliance():
    ...
    try:
        ...
        test_decrypt = encryption_service.decrypt_to_string(
            test_encrypt, "compliance_check"
        )
        ...
        passed_checks = sum(
            1 for check in compliance_checks.values() if check.get("compliance", False)
        )
    ...
    try:
        ...
        monitoring_data = {
            ...
            "overall_security_status": (
                "good"
                if (
                    ssl_status.get("valid")
                    and encryption_health
                    and ssl_expiry.get("days_remaining", 0) > 7
                )
            ...
            monitoring_data["security_alerts"].append(
                {
                    ...
                    "severity": (
                        "warning"
                        if ssl_expiry.get("days_remaining", 0) > 7
                        else "critical"
                    ),
                }
            )

...
from ..models.database import User, Message, Appointment, Conversation
...
async def websocket_endpoint(
    ...
    try:
        ...
        logger.info(f"🔌 WebSocket conectado: {user.nome} ({user.id}) -> {subscription_list}")
...
async def send_initial_data(
    ...
    try:
        ...
        if "dashboard" in subscriptions:
            ...
            await manager._send_to_connection(
                connection_id,
                RealtimeEventType.DASHBOARD_STATS_UPDATE,
                dashboard_stats
            )
...
async def handle_specific_message_types(
    ...
    try:
        if message_type == "send_message":
            ...
            await handle_mark_message_read(connection_id, message_data, user, db, manager)
        
        elif message_type == "update_appointment":
            await handle_update_appointment(connection_id, message_data, user, db, manager)
...
async def get_recent_appointments(db: Session, user: User, limit: int = 10) -> List[Dict[str, Any]]:
...
async def get_recent_messages(db: Session, user: User, limit: int = 20) -> List[Dict[str, Any]]:

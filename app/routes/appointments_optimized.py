...
async def get_appointments_optimized(
    ...
    """
    try:
        logger.info(f"Fetching optimized appointments - filters: user_id={user_id}, business_id={business_id}")
        ...
        if benchmark:
            ...
            old_time, new_time, improvement = await benchmark_service.benchmark_appointments_query(
                user_id=user_id,
                business_id=business_id,
                limit=min(limit, 50)  # Limit for benchmark
            )
            
            logger.info(f"Performance benchmark - Old: {old_time:.3f}s, New: {new_time:.3f}s, Improvement: {improvement:.1f}%")
        ...
        logger.info(f"Successfully retrieved {len(appointments)} optimized appointments")
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "completion_rate": (completed_count / len(appointments) * 100) if appointments else 0
            }
        }
        
    except Exception as e:
        logger.error(f"Error fetching user appointment history: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error fetching appointment history: {str(e)}")
...
async def get_appointment_analytics_optimized(
    ...
    """
    try:
        logger.info(f"Fetching optimized appointment analytics - business_id: {business_id}")

...
async def get_appointments_optimized(
    ...
    try:
        logger.info(
            f"🔍 Buscando agendamentos (OTIMIZADO) - Admin: {current_admin.username}"
        )
        ...
        logger.info(
            f"✅ P001: Encontrados {len(appointments)} agendamentos com joinedload"
        )
...
async def compare_query_performance(
    ...
    try:
        ...
        appointments_optimized = await get_appointments_with_relationships(
            session=session, limit=limit, offset=0
        )
        ...
        # Análise
        improvement_pct = (
            ((time_joins - time_joinedload) / time_joins) * 100 if time_joins > 0 else 0
        )
        ...
        return {
            "test_results": {
                ...
                "performance_analysis": {
                    ...
                    "recommendation": (
                        "joinedload" if time_joinedload < time_joins else "joins"
                    ),
                },
            },

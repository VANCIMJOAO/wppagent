...
COOKIE_CONFIG = {
    "httponly": True,
    "secure": False,  # Temporariamente False para desenvolvimento local
    "samesite": "strict",
    "path": "/",
}

# Armazenamento temporário de usuários (em produção usar banco de dados)
# NOTA: Credenciais hardcoded removidas por segurança
# Usuário admin deve ser criado no banco de dados
registered_users = {}
...
async def login(request: LoginRequest, http_request: Request, response: Response):
    ...
    if requires_2fa:
        ...
        temp_token = jwt_manager.create_access_token(user_id, role, ["2fa_pending"])
        ...
        return LoginResponse(
            ...
            requires_2fa=True,
            user_info={"user_id": user_id, "role": role, "2fa_required": True},
        )
...
async def setup_2fa(user: Dict = Depends(get_current_user_from_token)):
    ...
    return TwoFactorSetupResponse(
        secret=secret, qr_code=qr_code, backup_codes=[]  # Será gerado após confirmação
    )
...
async def verify_2fa(
    ...
    if "2fa_pending" not in user.get("permissions", []):
        raise HTTPException(status_code=400, detail="2FA verification not required")
...
async def refresh_token(http_request: Request, response: Response):
    ...
    try:
        ...
        # Atualizar cookies
        response.set_cookie(
            key="access_token", value=access_token, max_age=3600, **COOKIE_CONFIG
        )
...
async def revoke_token(
    ...
    if request.revoke_all:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid token or already revoked"
            )
...
async def get_backup_codes_count(user: Dict = Depends(get_current_user_from_token)):
    ...
    return {"backup_codes_remaining": count, "low_backup_codes": count <= 2}
...
async def security_events(limit: int = 50, user: Dict = Depends(require_admin)):

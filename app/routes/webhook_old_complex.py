...
Versão otimizada que substitui os múltiplos controles sobrepostos por um
sistema único e eficiente baseado em Redis.

MELHORIAS IMPLEMENTADAS:
1. ✅ Sistema unificado de controle via Redis + fallback
2. ✅ Eliminação de verificações redundantes no banco
3. ✅ Remoção de cache em arquivo temporário
4. ✅ TTL automático para mensagens processadas
5. ✅ Métricas precisas e performance otimizada
6. ✅ Detecção inteligente de mensagens duplicadas
"""
...
class ResponseControl:
    ...
    def load_cache(self):
        ...
        try:
            ...
            if os.path.exists(ACTIVE_RESPONSES_FILE):
                ...
                logger.info(
                    f"📂 Respostas ativas: {len(self.active_responses)} usuários"
                )
    ...
    def cleanup_old_entries(self):
        ...
        old_responses = [
            k for k, v in self.active_responses.items() if current_time - v > 600
        ]  # 10 minutos
        ...
        if old_cache or old_responses:
            logger.info(
                f"🧹 Limpeza: {len(old_cache)} cache + {len(old_responses)} respostas"
            )
            self.save_cache()
    ...
    async def _fallback_cache_check(
        ...
        async with self.processing_locks[user_id]:
            ...
            logger.debug(
                f"🔍 Verificação cache: {user_id} - {content[:30]}... - {message_key}"
            )
            ...
            if user_id in self.active_responses:
                ...
                if time_diff < 30:  # 30 segundos
                    self.stats["messages_blocked"] += 1
                    logger.warning(
                        f"🚫 RESPOSTA RECENTE CACHE: {user_id} - {time_diff:.1f}s"
                    )
...
Digite "mais serviços" para ver outras opções! 😊""",
    ...
    def generate_single_response(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["oi", "olá", "bom dia", "tudo bem"]):
            ...
            word in message_lower for word in ["serviço", "tratamento", "oferece"]
        ):
...
@router.post("/webhook")
async def receive_webhook_absolute(
...
async def cleanup_active_responses(current_time: float):
    ...
    try:
        if os.path.exists(ACTIVE_RESPONSES_FILE):
            ...
            old_responses = [
                phone
                for phone, timestamp in active_responses.items()
            ...
            if old_responses:
                ...
                logger.info(
                    f"🧹 Removidas {len(old_responses)} respostas ativas antigas"
                )

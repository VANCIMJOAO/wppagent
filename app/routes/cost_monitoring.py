        ...
        return {
            ...
            "data": {
                ...
                "usage_percentage": round(
                    (
                        (daily_cost_usd / cost_tracker.daily_limit_usd * 100)
                        if cost_tracker.daily_limit_usd > 0
                        else 0
                    ),
                    2,
                ),
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "limit_brl": cost_tracker.monthly_limit_usd
                * cost_tracker.exchange_rate,
                "usage_percentage": round(
                    (
                        (monthly_cost_usd / cost_tracker.monthly_limit_usd * 100)
                        if cost_tracker.monthly_limit_usd > 0
                        else 0
                    ),
                    2,
                ),
...
async def update_cost_settings(
    ...
    try:
        ...
        if daily_limit_usd is not None and daily_limit_usd <= 0:
            raise HTTPException(
                status_code=400, detail="Limite diário deve ser positivo"
            )

        if monthly_limit_usd is not None and monthly_limit_usd <= 0:
            raise HTTPException(
                status_code=400, detail="Limite mensal deve ser positivo"
            )

        if exchange_rate is not None and exchange_rate <= 0:
            raise HTTPException(
                status_code=400, detail="Taxa de câmbio deve ser positiva"
            )

        if alert_threshold is not None and (
            alert_threshold <= 0 or alert_threshold > 1
        ):
            raise HTTPException(
                status_code=400, detail="Threshold de alerta deve estar entre 0 e 1"
            )
        ...
        return {
            ...
            "data": {
                ...
                "daily_usage_pct": round(
                    (
                        (daily_cost / cost_tracker.daily_limit_usd * 100)
                        if cost_tracker.daily_limit_usd > 0
                        else 0
                    ),
                    1,
                ...
                "monthly_usage_pct": round(
                    (
                        (monthly_cost / cost_tracker.monthly_limit_usd * 100)
                        if cost_tracker.monthly_limit_usd > 0
                        else 0
                    ),
                    1,
...
async def get_cost_timeline(days: int = 30):
    ...
    try:
        ...
        return {"success": True, "data": timeline_data, "period_days": days}

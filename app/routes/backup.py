...
async def trigger_backup(
    ...
    try:
        ...
        logger.info(
            f"Manual backup triggered by {admin_user.username} - Type: {request.backup_type}"
        )
        ...
        if request.backup_type == "full" or request.backup_type is None:
            ...
            return {
                ...
                "message": (
                    ...
                    if result
                    else "Redis backup skipped (not available)"
                ),
                "backup_info": result,
                "timestamp": datetime.now().isoformat(),
            }
            ...
            return {
                ...
                "message": (
                    ...
                    if result
                    else "Files backup skipped (no files)"
                ),
                "backup_info": result,
                "timestamp": datetime.now().isoformat(),
            }
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid backup type: {request.backup_type}"
            )
...
async def cleanup_backups(
    force: bool = False, admin_user: AdminUser = Depends(get_current_admin_user)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Backup cleanup requested by {admin_user.username} - Force: {force}"
        )
...
async def download_backup(
    filename: str, admin_user: AdminUser = Depends(get_current_admin_user)
):
    """
    ...
    try:
        ...
        if not filename.endswith(".gz") or not any(
            t in filename for t in ["postgres", "redis", "files"]
        ):
...
async def get_backup_config(admin_user: AdminUser = Depends(get_current_admin_user)):
    ...
    try:
        ...
        config = {
            ...
            "cloud_enabled": os.getenv("BACKUP_CLOUD_ENABLED", "false").lower()
            == "true",
...
async def update_backup_schedule(
    ...
    try:
        ...
        logger.info(
            f"Backup schedule updated by {admin_user.username}: {config.cron_schedule}"
        )
        ...
        if len(parts) != 5:
            raise HTTPException(
                status_code=400, detail="Cron schedule must have 5 parts"
            )
...
async def get_backup_logs(
    lines: int = 100, admin_user: AdminUser = Depends(get_current_admin_user)
):
    """

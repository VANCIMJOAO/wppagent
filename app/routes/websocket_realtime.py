...
async def websocket_endpoint(
    websocket: WebSocket,
    token: str = Query(...),
    subscriptions: str = Query("dashboard,appointments,conversations")  # comma-separated
):
    """
    ...
    try:
        ...
        # Connect WebSocket
        connected = await websocket_manager.connect(websocket, user_id, subscription_list)
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.warning(f"Message processing error for {user_id}: {msg_error}")
...
async def websocket_health_check():
    ...
    return {
        ...
        "metrics": {
            ...
            "total_connections": stats["total_connections"],
            "stale_connections": stats["stale_connections"],
            "topics_active": len([t for t, count in stats["connections_by_topic"].items() if count > 0])
        },
        "server_time": datetime.utcnow().isoformat(),
        "uptime": "running"
    }

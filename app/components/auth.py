            ...
            dbc.DropdownMenu(
                [
                    ...
                    dbc.DropdownMenuItem(
                        [html.I(className="fas fa-user me-2"), "Meu Perfil"],
                        id="profile-menu",
                        n_clicks=0,
                    ),
                    dbc.DropdownMenuItem(
...
def create_auth_callbacks(app):
    ...
    def handle_login(n_clicks, username, password, remember, current_auth):
        ...
        if current_auth and current_auth.get("authenticated"):
            print(
                f"✅ Usuário já autenticado, ignorando callback: {current_auth.get('username')}"
            )
        ...
        print(
            f"🔐 Processando login: username='{username}', password={'*' * len(password) if password else 'None'}"
        )
        ...
        if admin_password and username == admin_username and password == admin_password:
            ...
            logger.error(
                "ADMIN_PASSWORD não configurada - configure as variáveis de ambiente ADMIN_USERNAME e ADMIN_PASSWORD"
            )
            return (
                dbc.Alert(
                    "Credenciais de administrador não configuradas. Configure ADMIN_PASSWORD nas variáveis de ambiente.",
                    color="danger",
                    className="mt-3",
                ),
                no_update,
        ...
        if login_successful:
            ...
            user_data = {
                "username": username,
                "full_name": (
                    "Admin User" if username == "admin" else "Environment Admin"
                ),
    ...
    @app.callback(Output("user-display-name", "children"), [Input("user-data", "data")])

...
class ConnectionManager:
    ...
    async def connect(
        ...
        try:
            ...
            logger.info(f"WebSocket connection accepted for user {user_id} in room {room}")
            ...
            welcome_message = WebSocketMessage(
                type=EventType.AUTH_REQUIRED.value if require_auth else EventType.AUTH_SUCCESS.value,
                data={
                    "user_id": connection_info.user_id,
                    "room": room,
                    "message": "Autenticação necessária" if require_auth else "Conectado com sucesso",
    ...
    async def send_to_connection(self, connection_info: ConnectionInfo, message: WebSocketMessage) -> bool:
        """
    ...
    async def broadcast_to_room(self, message: WebSocketMessage, room: str) -> int:
        ...
        logger.info(f"Broadcast to room {room}: {sent_count} successful, {len(failed_connections)} failed")
        return sent_count
    ...
    async def get_room_users(self, room: str) -> List[str]:
        ...
        return [
            conn.user_id for conn in self.active_connections[room] 

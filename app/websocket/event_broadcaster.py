...
from app.websocket.connection_manager import connection_manager, WebSocketMessage, EventType, RoomType
import logging
...
class EventBroadcaster:
    ...
    async def broadcast_appointment_deleted(self, appointment_id: int, appointment_data: dict = None):
        """
    ...
    async def broadcast_system_notification(self, message: str, level: str = "info", rooms: list = None):
        """
    ...
    async def broadcast_user_status_change(self, user_id: str, status: str, rooms: list = None):
        """
    ...
    async def broadcast_custom_event(self, event_type: str, data: dict, rooms: list = None):
        """
    ...
    async def _broadcast_event(self, event: dict):
        ...
        try:
            ...
            for room in rooms:
                ...
                logger.debug(f"Broadcast {event_type} to {room}: {sent_count} recipients")
            
            self.metrics["events_broadcasted"] += 1
            logger.info(f"Successfully broadcast {event_type} to {total_sent} connections across {len(rooms)} rooms")
    ...
    def get_metrics(self) -> dict:
        ...
        return {
            **self.metrics,
            ...
            "is_running": self.broadcasting_task is not None and not self.broadcasting_task.done()
        }
...
async def notify_appointment_deleted(appointment_id: int, appointment_data: dict = None):
    """Helper para notificar deleção de agendamento"""
    await event_broadcaster.broadcast_appointment_deleted(appointment_id, appointment_data)

...
async def test_webhook_signature_validation(client: AsyncClient):
    ...
    assert (
        ...
        or "protection" in error_data["detail"].lower()
    )
    ...
    assert response.status_code in [
        200,
        202,
    ], f"Valid webhook should be accepted, got {response.status_code}"
...
async def test_webhook_malformed_payload_handling(client: AsyncClient):
    ...
    malformed_signature = hmac.new(
        test_secret.encode("utf-8"), malformed_body.encode("utf-8"), hashlib.sha256
    ).hexdigest()
...
async def test_webhook_concurrent_processing(client: AsyncClient):
    ...
    ), "At least some concurrent webhooks should succeed"

    # No exceptions should occur during processing
    assert (
        len(exceptions) == 0
    ), f"Concurrent processing caused exceptions: {exceptions}"

    # Server should not return 500 errors
    server_errors = [r for r in failed_responses if r.status_code == 500]
    assert (
        len(server_errors) == 0
    ), "Concurrent processing should not cause server errors"
...
async def test_webhook_payload_size_limits(client: AsyncClient):
    ...
    normal_payload = {"test": "payload_size", "data": "A" * 100}  # 100 characters
    ...
    large_payload = {"test": "large_payload_size", "data": "B" * 10000}  # 10KB of data
...
async def test_webhook_signature_timing_attack_protection(client: AsyncClient):
    ...
    for sig in incorrect_signatures:
        ...
        response = await client.post(
            "/webhook", json=test_payload, headers={"X-Hub-Signature-256": sig}
        )
        end_time = time.time()
    ...
    if len(response_times) > 1:
        ...
        ), "Response times vary too much - possible timing attack vulnerability"
...
async def test_webhook_signature_validation(
    client: AsyncClient, webhook_payload: Dict[str, Any], webhook_secret: str
):
    """
    ...
    response = await client.post(
        "/webhook", json=webhook_payload, headers=invalid_headers
    )
    assert response.status_code in [401, 403]
    ...
    response = await client.post(
        "/webhook", json=webhook_payload, headers=valid_headers
    )
...
@pytest.mark.integration
@pytest.mark.webhook
async def test_webhook_payload_processing(client: AsyncClient, webhook_secret: str):
    ...
    response = await client.post(
        "/webhook", json=image_message_payload, headers=headers
    )
    assert response.status_code in [200, 202]
...
@pytest.mark.integration
@pytest.mark.webhook
async def test_webhook_malformed_payloads(client: AsyncClient, webhook_secret: str):
    ...
    invalid_payload = {"invalid": "structure", "random": "data"}
...
async def test_webhook_security_headers(
    ...
    response = await client.post(
        "/webhook", json=webhook_payload, headers=malformed_headers
    )
    assert response.status_code in [401, 403]
    ...
    response = await client.post(
        "/webhook", json=webhook_payload, headers=no_prefix_headers
    )
    assert response.status_code in [401, 403]
...
@pytest.mark.webhook
@pytest.mark.load
async def test_webhook_concurrent_processing(client: AsyncClient, webhook_secret: str):
                                ...
                                "messages": [
                                    {
                                        ...
                                        "type": "text",
                                        "text": {
                                            "body": f"Concurrent test message {message_id}"
                                        },
                                    }
                                ],
                            },

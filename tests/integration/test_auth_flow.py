...
async def test_complete_auth_flow(client: AsyncClient):
    ...
    if login_response.status_code not in [200, 201]:
        ...
        pytest.skip(
            f"Login failed with {login_response.status_code} - test user may not exist"
        )
    ...
    if not auth_cookie_found:
        pytest.skip(
            "No authentication cookie found - may indicate different auth implementation"
        )

    # 3. ACCESS PROTECTED ENDPOINT - Test cookie-based authentication
    protected_endpoints = ["/dashboard/stats", "/appointments/", "/auth/me"]
    ...
    post_logout_response = await client.get(
        "/dashboard/stats", cookies=post_logout_cookies
    )
...
async def test_httponly_cookie_security(client: AsyncClient):
    ...
    for cookie_header in set_cookie_headers:
        ...
        if any(keyword in cookie_lower for keyword in ["token", "auth", "session"]):
            ...
            assert (
                "secure" in cookie_lower
            ), f"Cookie missing Secure flag: {cookie_header}"
            assert (
                "samesite" in cookie_lower
            ), f"Cookie missing SameSite flag: {cookie_header}"
...
async def test_auth_validation_errors(client: AsyncClient):
    ...
    response = await client.post(
        "/auth/login", json={"email": "invalid-email-format", "password": "testpass"}
    )
    assert response.status_code in [400, 422]
    ...
    for endpoint in endpoints_to_test:
        ...
        if response.status_code not in [404, 501]:
            assert response.status_code not in [
                401,
                403,
            ], f"Session invalid for {endpoint}"
...
async def test_login_invalid_credentials(client: AsyncClient):
    ...
    # Wrong password
    response = await client.post(
        "/auth/login", json={"email": "vl001@test.com", "password": "wrongpassword"}
    )

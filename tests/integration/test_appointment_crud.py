...
async def test_appointment_crud_complete_flow(
    ...
    update_data = {
        "notes": "VL-001 Updated test appointment", 
        "status": "confirmado"
    }
...
async def test_appointment_list_with_filters(
    ...
    paginated_response = await authenticated_client.get("/appointments/?limit=2&offset=0")
...
async def test_appointment_business_logic(
    ...
    create_response = await authenticated_client.post("/appointments/", json=future_date_data)
...
async def test_appointment_concurrent_operations(
    ...
    create_response = await authenticated_client.post("/appointments/", json=appointment_data)
...
async def test_appointment_edge_cases(
    ...
    response = await authenticated_client.post("/appointments/", json=special_chars_data)
    assert response.status_code in [200, 201, 400, 422]

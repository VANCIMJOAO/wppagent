...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "welcome_message",
            sa.Text(),
            server_default="Olá! 👋 Bem-vindo à nossa empresa! Como posso ajudá-lo hoje?",
        ),
        sa.Column(
            "auto_response_enabled", sa.Boolean(), server_default=sa.text("true")
        ),
    ...
    op.create_index(
        op.f("ix_message_templates_id"), "message_templates", ["id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "auto_confirm_bookings", sa.Boolean(), server_default=sa.text("false")
        ),
        sa.Column("slot_duration_minutes", sa.Integer(), server_default="30"),
        sa.Column(
            "break_between_appointments_minutes", sa.Integer(), server_default="0"
        ),
        sa.Column(
            "send_confirmation_messages", sa.Boolean(), server_default=sa.text("true")
        ),
        sa.Column(
            "send_reminder_messages", sa.Boolean(), server_default=sa.text("true")
        ),
        ...
        sa.Column(
            "require_phone_confirmation", sa.Boolean(), server_default=sa.text("false")
        ),
    ...
    op.create_index(
        op.f("ix_bot_configurations_id"), "bot_configurations", ["id"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_available_slots_id"), "available_slots", ["id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "collection_status", sa.String(length=20), server_default="incomplete"
        ),
    ...
    op.create_index(
        op.f("ix_customer_data_collection_id"),
        "customer_data_collection",
        ["id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_conversation_contexts_id"),
        "conversation_contexts",
        ["id"],
        unique=False,
    )
...
def downgrade():
    ...
    op.drop_index(
        op.f("ix_conversation_contexts_id"), table_name="conversation_contexts"
    )
    op.drop_table("conversation_contexts")

    op.drop_index(
        op.f("ix_customer_data_collection_id"), table_name="customer_data_collection"
    )

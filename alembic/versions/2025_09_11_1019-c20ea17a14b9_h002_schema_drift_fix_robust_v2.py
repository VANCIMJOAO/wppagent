...
def upgrade() -> None:
    ...
    try:
        ...
        op.execute(
            "ALTER TABLE auth_users ALTER COLUMN last_login TYPE TIMESTAMPTZ USING last_login::timestamptz"
        )
    ...
    try:
        ...
        op.execute(
            "ALTER TABLE business_hours ADD COLUMN IF NOT EXISTS temp_open_time VARCHAR(5)"
        )
        op.execute(
            "ALTER TABLE business_hours ADD COLUMN IF NOT EXISTS temp_close_time VARCHAR(5)"
        )
        op.execute(
            "ALTER TABLE business_hours ADD COLUMN IF NOT EXISTS temp_break_start_time VARCHAR(5)"
        )
        op.execute(
            "ALTER TABLE business_hours ADD COLUMN IF NOT EXISTS temp_break_end_time VARCHAR(5)"
        )

        # Copiar dados convertidos
        op.execute(
            "UPDATE business_hours SET temp_open_time = open_time::TEXT WHERE open_time IS NOT NULL"
        )
        op.execute(
            "UPDATE business_hours SET temp_close_time = close_time::TEXT WHERE close_time IS NOT NULL"
        )
        op.execute(
            "UPDATE business_hours SET temp_break_start_time = break_start_time::TEXT WHERE break_start_time IS NOT NULL"
        )
        op.execute(
            "UPDATE business_hours SET temp_break_end_time = break_end_time::TEXT WHERE break_end_time IS NOT NULL"
        )
        ...
        op.execute(
            "ALTER TABLE business_hours RENAME COLUMN temp_open_time TO open_time"
        )
        op.execute(
            "ALTER TABLE business_hours RENAME COLUMN temp_close_time TO close_time"
        )
        op.execute(
            "ALTER TABLE business_hours RENAME COLUMN temp_break_start_time TO break_start_time"
        )
        op.execute(
            "ALTER TABLE business_hours RENAME COLUMN temp_break_end_time TO break_end_time"
        )

        # Add new index
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_business_hours_id ON business_hours (id)"
        )
    ...
    try:
        op.execute(
            "ALTER TABLE business_policies ALTER COLUMN policy_type TYPE VARCHAR(100)"
        )
        ...
        # Add new index
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_business_policies_id ON business_policies (id)"
        )
    ...
    try:
        ...
        op.execute(
            "ALTER TABLE login_attempts ALTER COLUMN attempted_at TYPE TIMESTAMPTZ USING attempted_at::timestamptz"
        )
        ...
        op.execute(
            "CREATE INDEX IF NOT EXISTS ix_login_attempts_id ON login_attempts (id)"
        )
    ...
    try:
        ...
        op.execute(
            "ALTER TABLE services ALTER COLUMN price TYPE VARCHAR(20) USING price::TEXT"
        )

...
def upgrade():
    ...
    auth_users_exists = connection.execute(text("""
        SELECT EXISTS (
    ...
    admin_users_exists = connection.execute(text("""
        SELECT EXISTS (
            SELECT 1 FROM information_schema.tables 
    ...
    print(f"HF003 - Status: auth_users={auth_users_exists}, admin_users={admin_users_exists}")
    ...
    if admin_users_exists and auth_users_exists:
        ...
        if migrated_count == 0:
            ...
            admin_columns = connection.execute(text("""
            ...
            if 'name' in admin_column_names:
                ...
                name_field = "SPLIT_PART(au.email, '@', 1)"  # Usar parte do email como nome

...
def upgrade() -> None:
    ...
    op.add_column(
        "appointments", sa.Column("end_time", sa.DateTime(timezone=True), nullable=True)
    )
    ...
    op.add_column(
        "appointments", sa.Column("cancelled_by", sa.String(length=20), nullable=True)
    )
    op.add_column(
        "appointments",
    ...
    op.add_column(
        "appointments", sa.Column("confirmed_by", sa.String(length=20), nullable=True)
    )
    op.add_column(
        "appointments",
        sa.Column("price_at_booking", sa.String(length=20), nullable=True),
    )

    # Atualizar appointments existentes com business_id e service_id padrão
    op.execute(
        "UPDATE appointments SET business_id = 1, service_id = 1 WHERE business_id IS NULL"
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "appointments",
        sa.Column(
            "service", sa.VARCHAR(length=255), autoincrement=False, nullable=False
        ),
    )

...
export class WhatsAgentApiClient {
    ...
    async getAppointmentsBefore(): Promise<AppointmentList> {
        // ANTES CF002 - resposta inconsistente, estrutura diferente
        return this.apiCall<AppointmentList>('/appointments-demo/before');
    }
    ...
    async createAppointment(appointmentData: Partial<Appointment>): Promise<{appointment: Appointment, message: string}> {
        return this.apiCall<{appointment: Appointment, message: string}>('/appointments-demo/create-demo', {
            method: 'POST',
    ...
    async getMetrics(): Promise<any> {
        return this.apiCall<any>('/health-demo/metrics');
    }

    // ⚠️ CF002 Demo - Error Handling
    async triggerNotFoundError(): Promise<any> {
        // Este método sempre lança erro - para demonstrar error handling
        return this.apiCall<any>('/appointments-demo/error-demo');
    }

    async triggerValidationError(): Promise<any> {
        return this.apiCall<any>('/appointments-demo/validation-error-demo');
    }

    async triggerServerError(): Promise<any> {
        return this.apiCall<any>('/appointments-demo/server-error-demo');
    }
}
...
export const AppointmentListComponent: React.FC = () => {
    const client = React.useMemo(() => new WhatsAgentApiClient(), []);
    ...
    return (
        <>
            ...
            {!loading && !error && !appointments && <div>Nenhum agendamento encontrado</div>}

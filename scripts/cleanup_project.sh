...
echo "🗑️ Removendo migrações Alembic duplicadas vazias específicas..."
# Remover migrações conhecidas que são duplicatas e vazias
rm -f alembic/versions/remove_duplicate_admin_2025.py 2>/dev/null || true
rm -f alembic/versions/add_orphan_indexes_2025.py 2>/dev/null || true
rm -f alembic/versions/2025_09_12_1300_pd002_schema_cleanup.py 2>/dev/null || true
...
ALEMBIC_EMPTY=$(find alembic/versions/ -type f -size 0 2>/dev/null | wc -l)
if [ "$ALEMBIC_EMPTY" -eq 0 ]; then
    echo "✅ Nenhuma migração vazia no Alembic"
else
    echo "⚠️  $ALEMBIC_EMPTY migrações vazias encontradas:"
    find alembic/versions/ -type f -size 0 2>/dev/null
fi

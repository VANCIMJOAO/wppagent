...
export default function ConversationsPage() {
  ...
  const parseAnalyticsData = (data: any): Contact[] => {
    ...
    // Se são dados de lista de conversas (dados reais - prioridade)
    if (data?.conversations && Array.isArray(data.conversations)) {
      console.log('✅ Usando lista real de conversas do PostgreSQL:', data.conversations.length);
      return data.conversations.map((conv: any) => {
        ...
        // Status visual baseado na atividade
        const isRecent = conv.last_message_at && 
          (Date.now() - new Date(conv.last_message_at).getTime()) < 24 * 60 * 60 * 1000; // últimas 24h
        
        return {
          id: conv.id.toString(),
          name: conv.user_name || `Usuário ${conv.user_id}`,
          phone: conv.user_phone || conv.phone_number || 'N/A',
          lastMessage: conv.last_message || `${conv.total_messages} mensagens`,
          timestamp: lastMessageTime,
          unreadCount: conv.unread_messages || 0,
          status: (conv.status === 'active' && isRecent ? 'online' : 'offline') as any,
          tags: ['PostgreSQL']
        };
      });
    }
    
    // Se são dados do /conversations/stats/summary (PostgreSQL)
    if (data?.total_conversations !== undefined) {
      console.log('ℹ️ Usando dados estatísticos do PostgreSQL');
      return [{
        id: 'postgresql-stats',
        name: '📊 Estatísticas do PostgreSQL',
        phone: 'PostgreSQL Database',
        lastMessage: `${data.total_conversations} conversas, ${data.total_messages} mensagens, ${data.active_conversations} ativas`,
        ...
        tags: ['PostgreSQL', 'Stats']
      }];
    }
    
    // Se são dados do /conversation/analytics (memória)
    if (data?.conversation_details && typeof data.conversation_details === 'object') {
      console.log('ℹ️ Usando dados de memória');

...
export default function ConversationsPage() {
  ...
  const [loading, setLoading] = useState(false);  // Começar false pois temos mock
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log('📊 Resposta do backend:', conversationsResponse);
        
        // ✅ VERIFICAÇÃO ROBUSTA DOS DADOS
        if (conversationsResponse?.success && conversationsResponse?.data && Array.isArray(conversationsResponse.data)) {
          const conversations = conversationsResponse.data;
          
          if (conversations.length > 0) {
            // Converter dados do backend para formato frontend
            const contactsData = conversations.map(convertConversationToContact).filter(Boolean);
            ...
            if (contactsData.length > 0) {
              ...
              // Carregar mensagens para o primeiro contato
              try {
                const messagesResponse = await getConversationMessages(firstContact.id);
                ...
                if (messagesResponse?.success && messagesResponse?.data && Array.isArray(messagesResponse.data)) {
                  ...
                  const messagesData = backendMessages.map(convertMessageToConversationMessage).filter(Boolean);
                  setMessages(messagesData);
                  console.log('✅ Dados reais carregados com sucesso');
                } else {
                  console.warn('⚠️ Mensagens inválidas, usando mock para mensagens');
                  setMessages(mockMessages);
                }
              } catch (msgError) {
                console.warn('⚠️ Erro ao carregar mensagens, usando mock:', msgError);
                setMessages(mockMessages);
              }
        ...
        toast.info('Carregado em modo demonstração');
        ...
        console.log('🔄 Aplicando fallback para dados mock');
        setContacts(mockContacts);
        setMessages(mockMessages);
        setSelectedContact(mockContacts[0]);
        toast.error('Erro de conexão - Modo demonstração ativado');
  ...
  // ✅ VERIFICAÇÃO SEGURA PARA FILTROS
  const filteredContacts = Array.isArray(contacts) ? contacts.filter(contact => 
    contact?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    contact?.phone?.includes(searchTerm)
  ) : [];

  const contactMessages = Array.isArray(messages) ? messages.filter(msg => 
  ...
  const loadMessagesForContact = async (contact: Contact) => {
    ...
    try {
      setLoading(true);
      const messagesResponse = await getConversationMessages(contact.id);
      ...
      if (messagesResponse?.success && messagesResponse?.data && Array.isArray(messagesResponse.data)) {
        ...
        const mockMessagesForContact = mockMessages.filter(msg => msg.contactId === contact.id);
        setMessages(mockMessagesForContact.length > 0 ? mockMessagesForContact : mockMessages);
      }
    } catch (error) {
      console.error('Erro ao carregar mensagens:', error);
      toast.error('Erro ao carregar mensagens');
      // Usar mensagens mock como fallback
      const mockMessagesForContact = mockMessages.filter(msg => msg.contactId === contact.id);
      setMessages(mockMessagesForContact.length > 0 ? mockMessagesForContact : mockMessages);
    } finally {
      setLoading(false);

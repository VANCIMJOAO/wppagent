...
// ✅ FUNÇÃO PARA FETCH COM AUTENTICAÇÃO JWT DO SISTEMA EXISTENTE
async function fetchWithAuth(endpoint: string, options: RequestInit = {}) {
  ...
  const response = await fetch(`https://wppagent-production.up.railway.app${endpoint}`, {
    ...options,
    headers: {
...
export default function ConversationsPage() {
  ...
  const loadConversations = async () => {
    try {
      ...
      // ✅ ENDPOINT CORRETO: /conversations/ (com JWT)
      const data = await fetchWithAuth('/conversations/?limit=50&offset=0');
      
      if (data?.conversations && Array.isArray(data.conversations)) {
        const backendContacts = data.conversations.map((conv: any) => ({
          id: conv.id.toString(),
          name: conv.user_name || conv.user_phone || 'Sem nome',
          phone: conv.user_phone || 'N/A',
          lastMessage: conv.last_message || 'Sem mensagens',
          timestamp: new Date(conv.last_message_at || conv.created_at).toLocaleTimeString('pt-BR', {
            hour: '2-digit',
            minute: '2-digit'
          }),
          unreadCount: conv.unread_messages || 0,
          status: (conv.status === 'active' ? 'online' : 'offline') as any,
          tags: ['Cliente']
        }));
        ...
        if (backendContacts.length > 0) {
          ...
          await loadMessages(backendContacts[0].id);
        } else {
          console.log('ℹ️ Nenhuma conversa no backend, usando mock');
        }
      } else {
        console.log('⚠️ Dados inválidos do backend, mantendo mock');
      }
    } catch (error) {
    ...
    try {
      ...
      if (data?.messages && Array.isArray(data.messages)) {
        const backendMessages = data.messages.map((msg: any) => ({
          ...
          isFromMe: msg.direction === 'out', // ✅ Conversão padronizada usando direction
          type: msg.message_type || 'text',
          status: 'delivered' as any
        }));

        console.log('✅ Mensagens carregadas:', backendMessages.length);

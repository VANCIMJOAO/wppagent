/**
 * 🪝 Hook de Autenticação com Cookies Seguros (Consolidado)
 * =========================================================
 *
 ...
 * - Provider para compartilhar estado globalmente
 *
 * ✅ Consolidado de useAuth.ts e useAuth-secure.tsx
 * 🔒 Thread-safe com SecureAuthManager
 */
...
export function useAuth(): UseAuthReturn {
  ...
  useEffect(() => {
    const checkInitialAuth = async () => {
      try {
        ...
        debugLog.error('🔴 Erro ao verificar auth inicial:', error);
        setAuthState(prev => ({
          ...prev,
          isAuthenticated: false,
          user: null,
  ...
  const refreshUser = useCallback(async (): Promise<void> => {
    try {
      ...
      debugLog.error('🔴 Erro ao atualizar usuário:', error);
      setAuthState(prev => ({
        ...prev,
 ...
 * @example
 * ```typescript
 * // Em qualquer componente dentro de AuthProvider
